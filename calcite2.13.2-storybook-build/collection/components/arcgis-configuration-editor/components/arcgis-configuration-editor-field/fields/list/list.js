...
import { cloneObject, getProp, getWithDefault, mergeObjects, setProp } from '@esri/hub-common';
import { bind } from '../../../../../../utils/context';
import intlManager from '../../../../../../utils/intl-manager';
import { getDefaultEditSchema, getDefaultEditUiSchema } from './schemas';
import { ListEditModeTypes } from './resources';
...
export class SimpleList {
  constructor() {
    ...
    this.mergeUpdatedItems = (listItemValues) => {
      let updatedItems = cloneObject(this._items);
      ...
      // if we have values, then we actually apply the changes and merge the item into the updated list
        ...
        if (this._scopeWrapperProp && !this._editProp) {
          mergeObjects(values, updatedItems[itemIndex], Object.keys(listItemValues[this._scopeWrapperProp]));
        }
        else if (this._editProp) {
          mergeObjects(values, updatedItems[itemIndex], [this._editProp]);
        }
        else {
    ...
    this.renderAddItemButton = () => {
      return (h("calcite-button", { appearance: "outline", disabled: this._isDisabled, onClick: this.handleItemEdit, round: true }, this._addItemLabel));
    };
    this.params = undefined;
    this.showEditExperience = false;
    this._editingItemKey = '';
    bind(this, 'handleListReorder', 'toggleItemVisibility', 'handleItemEdit', 'handleEditApplied', 'handleItemRemove', 'renderListItem', 'translationFunction');
  }
  ...
  get _editValues() {
    ...
    const editValues = this._editProp ? getProp(itemToEdit, this._editProp) : itemToEdit;
  ...
  get _editLabel() {
    const itemToEdit = this._items.find(c => c.key === this._editingItemKey);
    return (itemToEdit === null || itemToEdit === void 0 ? void 0 : itemToEdit.label) || getWithDefault(this.params, 'uiSchema.options.defaultEditLabel', '');
  }
  /**
   * The label for the add item button, if rendered. Defaults to "Add item".
   */
  get _addItemLabel() {
    return getWithDefault(this.params, 'uiSchema.options.addItemLabel', this.intl.t("addItem"));
  }
  /**
   * The prop to use for editing the list item, if any was passed in.
   */
  get _editProp() {
    return getProp(this.params, "uiSchema.options.editProp");
  }
  /**
  ...
  get _scopeWrapperProp() {
    return getProp(this.params, "uiSchema.options.scopeWrapperProp") || getProp(this.params, "uiSchema.options.editProp");
  }
  /**
   * The edit mode for the list field.
   */
  get _editMode() {
    return getProp(this.params, "uiSchema.options.editMode") || ListEditModeTypes.MODAL;
  }
  /**
  ...
  get _editActionIcon() {
    return {
      [ListEditModeTypes.FLOW]: 'chevron-right',
      [ListEditModeTypes.MODAL]: 'pencil'
  ...
  get _flowTarget() {
    ...
    const flowTargetCallback = getProp(this.params, "uiSchema.options.calciteFlowRefCallback");
   ...
   */
  get _allowHide() {
    return !this._isDisabled && !!getProp(this.params, 'uiSchema.options.allowHide');
  }
  /**
   ...
   */
  get _hiddenEditProp() {
    return getProp(this.params, 'uiSchema.options.hiddenEditProp');
  }
  /**
   * Determines if the list items can be reordered.
   */
  get _allowReorder() {
    return !this._isDisabled && !!getProp(this.params, 'uiSchema.options.allowReorder');
  }
  /**
   * Determines if the list items can be edited.
   */
  get _allowEdit() {
    return !this._isDisabled && !!getProp(this.params, 'uiSchema.options.allowEdit');
  }
  /**
   * Determines if the list items can be removed.
   */
  get _allowRemove() {
    return !this._isDisabled && !!getProp(this.params, 'uiSchema.options.allowRemove');
  }
  /**
   * Determines if new list items can be added
   */
  get _allowAdd() {
    return !this._isDisabled && !!getProp(this.params, 'uiSchema.options.allowAdd');
  }
  /**
   * The telemetry event to fire when adding an item, if any
   */
  get _addItemTelemetry() {
    return getProp(this.params, 'uiSchema.options.addItemTelemetry');
  }
  /**
   ...
   */
  get _editSchema() {
    return getProp(this.params, "uiSchema.options.editSchema") || getDefaultEditSchema({ maxLabelLength: getProp(this.params, 'uiSchema.options.maxLabelLength') });
  }
  /**
   ...
   */
  get _editUiSchema() {
    return getProp(this.params, "uiSchema.options.editUiSchema") || getDefaultEditUiSchema({ maxLabelLength: getProp(this.params, 'uiSchema.options.maxLabelLength'), intl: this.intl });
  }
  /**
  ...
  _isItemHidden(item) {
    ...
    if (this._hiddenEditProp) {
      ...
      const editValues = getProp(itemToEdit, this._hiddenEditProp) || {};
  ...
  handleListReorder(event) {
    ...
    const reorderedItems = cloneObject(this._items);
  ...
  toggleItemVisibility(event) {
    ...
    const updatedItems = cloneObject(this._items);
    ...
    if (this._hiddenEditProp) {
      setProp(`${this._hiddenEditProp}.hidden`, !getProp(itemToToggle, `${this._hiddenEditProp}.hidden`), itemToToggle);
    }
  ...
  handleItemRemove(event) {
    const key = event.target.dataset.key;
    const updatedItems = cloneObject(this._items).filter(item => item.key !== key);
    this.arcgisConfigurationEditorFieldInputChange.emit(updatedItems);
  }
  /**
  ...
  handleEditApplied(event) {
    ...
    if (this._editMode === ListEditModeTypes.MODAL) {
      this.closeEditingExperience();
    }
  }
  /**
   ...
   */
  renderItemEditModal() {
    return h("arcgis-wormhole", null, h("arcgis-configuration-form", { isOpen: this.showEditExperience, layout: "modal", messageOverrides: { save: this.intl.t('itemEditModal.apply') }, onArcgisConfigurationFormModalClosed: this.closeEditingExperience, onArcgisConfigurationFormSaved: this.handleEditApplied, scale: this.params.scale, schema: this._editSchema, t: this.translationFunction, uiSchema: this._editUiSchema, values: this._editValues, variant: getProp(this.params, "uiSchema.options.variant") }, this.showEditExperience && h("div", { slot: "header" }, this.intl.t('editItem', { itemLabel: this._editLabel }))));
  }
  /**
  ...
  renderItemEditInline() {
    return h("arcgis-configuration-editor", { onArcgisConfigurationEditorChange: this.handleEditApplied, schema: this._editSchema, t: this.translationFunction, uiSchema: this._editUiSchema, values: this._editValues, variant: getProp(this.params, "uiSchema.options.variant") });
  }
  /**
   ...
   */
  renderListItem(item) {
    return h("calcite-list-item", { closable: this._allowRemove, "data-key": item.key, description: item.description, disabled: this._isDisabled, key: item.key, label: item.label, onCalciteListItemClose: this.handleItemRemove, onClick: this.handleItemEdit, selected: this._allowHide && !this._isItemHidden(item) }, this._allowHide && (h("calcite-action", { "data-key": item.key, icon: this._isItemHidden(item) ? 'view-hide' : 'view-visible', onClick: this.toggleItemVisibility, scale: this.params.scale, slot: "actions-start", text: this.intl.t(this._isItemHidden(item) ? 'showItem' : 'hideItem', { itemLabel: item.label }) })), this._allowEdit && (h("calcite-action", { "data-key": item.key, icon: this._editActionIcon, onClick: this.handleItemEdit, scale: this.params.scale, slot: "actions-end", text: this.intl.t('editItem', { itemLabel: item.label }) })));
  }
  /**
   ...
   */
  renderList() {
    return (h("calcite-list", { dragEnabled: this._allowReorder, onCalciteListOrderChange: this.handleListReorder, scale: this.params.scale }, this._items.map(this.renderListItem), this._allowAdd && this.renderAddItemButton()));
  }
  /**
  ...
  renderWormholedEditFlowList() {
    return (h(Fragment, null, h("calcite-flow-item", null, this.renderList()), (this.showEditExperience && this._flowTarget) &&
      h("arcgis-wormhole", { includeWormholeElement: false, target: this._flowTarget }, h("calcite-flow-item", { description: this._editLabel, heading: getProp(this.params, "uiSchema.options.flowTitle"), onCalciteFlowItemBack: this.closeEditingExperience }, this.renderItemEditInline()))));
  }
  /**
   ...
   */
  renderEditFlowList() {
    return (h("calcite-flow", null, h("calcite-flow-item", null, this.renderList()), this.showEditExperience &&
      h("calcite-flow-item", { heading: this._editLabel, onCalciteFlowItemBack: this.closeEditingExperience }, this.renderItemEditInline())));
  }
  /**
   ...
   */
  renderEditModalList() {
    return h(Fragment, null, this.renderItemEditModal(), this.renderList());
  }
  /**
  ...
  render() {
    return (h(Host, { "data-element": "list-field" }, this.renderListField()));
  }
  static get is() { return "hub-field-input-list"; }
  ...
  static get properties() {
    return {
      "params": {
        ...
        "docs": {
          "tags": [],
          "text": "Parameters passed from the editor."
        }
      }
    };
  }
