...
export const FILE_FORMATS = ['csv', 'shapefile', 'geojson', 'filegdb', 'featureCollection', 'excel', 'geoPackage', 'sqlite', 'json'];
export class CreateReplicaOptionsBuilder {
  constructor() {
...
export async function getDownloadUrlFromService(opts) {
...
export async function getTotalRecordCount(serviceUrl, options, authentication) {
...
export async function storeCreateReplicaError(hubUrl, opts) {
  try {
...
export const shouldRecordDownloadErrors = (appContext) => {
 ...
 */
export async function tryAuthenticatedGet(url, requestOptions) {
...
export const isNumberArray = (x) => {
  return Array.isArray(x) && x.every((el) => !isNaN(el));
};
export const isILayerOptionsArray = (x) => {
  return Array.isArray(x) && x.every(el => !isNaN(el.id));
};
export const isAGORateLimitError = (error) => error.name === 'ArcGISRequestError' && error.code === 429;
export const fileFormatToDisplayName = (format) => {
  return {
    csv: "CSV",
    shapefile: "Shapefile",
