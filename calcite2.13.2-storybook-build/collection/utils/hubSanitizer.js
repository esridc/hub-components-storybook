...
import { escapeAttrValue, safeAttrValue } from 'xss';
/**
 ...
 * @param {string} text text to sanitize
 */
export function sanitizePreviewMarkdown(text) {
  // hubTextCard + noHrefs
 ...
 * @param {string} text text to sanitize
 */
export function sanitizeHtml(text, rulesetName = 'hubTextCard') {
  ...
  let ruleset = getProp(HubSanitizerRules, rulesetName);
  if (ruleset) {
 ...
 */
export function validateHtml(text, rulesetName = 'hubTextCard') {
 ...
 * @param {string} text text to sanitize
 */
export function onlyLinks(text) {
  return _sanitize(text, HubSanitizerRules.linksOnly, false);
}
export function sanitizeHeadContent(text, page = false) {
  let metaTags = '';
  if (text) {
    ...
    if (metaTags) {
      ...
      const remainingMetaTags = Array.from(tempHead.childNodes).filter(node => getProp(node, 'tagName') === 'META');
...
function _sanitize(text, overrides, extendDefaults = true) {
  ...
  const s = new Sanitizer(overrides, extendDefaults);
 ...
 */
export function sanitizeUrl(value = '', forceSsl = true) {
 ...
 * @returns
 */
export function hubSafeAttrValue(tag, name, value, cssFilter) {
