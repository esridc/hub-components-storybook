...
import { NUMERIC_FIELD_TYPES } from "./feature-service";
 ...
 */
export function getMetricId(metricDisplay) {
 ...
 */
export function metricToStatCardConfig(resolvedMetric, metricDisplay, intl) {
  ...
  if (resolvedMetric === null || resolvedMetric === void 0 ? void 0 : resolvedMetric.features) {
    ...
    if (metricFeature) {
      ...
      else if (((!isNaN(parseFloat(value))) || NUMERIC_FIELD_TYPES.includes(metricDisplay === null || metricDisplay === void 0 ? void 0 : metricDisplay.fieldType)) && intl.formatNumber(value) !== "NaN") {
 ...
 *
 */
export function getSingleMetricFeature(resolvedMetric, metricId) {
 ...
 */
export function buildWhereClause(expressionSet = []) {
  return expressionSet.map(expression => {
    ...
    switch (field.type) {
      case "esriFieldTypeString":
        ...
        clause = relationship === ExpressionRelationships.LIKE ?
 ...
 */
export function transformEditorValuesToMetricAndCardConfig(values, metricId, opts) {
