import { getTypeFromEntity, shareEntityWithGroups, unshareEntityWithGroups } from "@esri/hub-common";
/**
 ...
 * @returns
 */
export async function unshareEntitiesToGroups(groups, entities, context) {
 ...
 * @returns
 */
export async function shareEntitiesToGroups(groups, entities, context) {
  const nested = await Promise.all(groups.map(async (g) => {
    // for each entity...
    return await Promise.all(entities.map(async (e) => {
      const result = { id: e.id, groupId: g.id, status: "success" };
