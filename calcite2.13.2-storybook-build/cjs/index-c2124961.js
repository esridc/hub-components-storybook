...
const remove$1 = require('./remove-921f5dc7.js');
...
function canUserCreateTeamInProduct(user, product, template) {
    ...
    const userGroups = getProp.getProp(user, "groups") || [];
    // can this be created in the current environment?
    if (userGroups.length <= userGroupLimit &&
        compose.includes(template.config.availableIn, product)) {
...
function removeInvalidPrivs(user, subscriptionInfoType) {
    // Clone User
    const clonedUser = util.cloneObject(user);
    ...
    if (!compose.includes(allowedSubscriptionTypes, subscriptionInfoType)) {
        clonedUser.privileges = util.without(clonedUser.privileges, "portal:user:addExternalMembersToGroup");
    }
    return clonedUser;
}
...
function applyPrivPropValuesToTemplate(user, template) {
    const templateCopy = util.cloneObject(template);
    // Only two templates actually have this in config atm, so we want to be safe.
    const ppv = getWithDefault.getWithDefault(template, "config.privPropValues", []);
    ...
    ppv.forEach((entry) => {
        ...
        if (compose.includes(user.privileges, entry.priv)) {
...
function getUserCreatableTeams(user, environment, subscriptionInfoType = "") {
    ...
    return util.cloneObject(teams).reduce((acc, teamTmpl) => {
...
function _translateTeamTemplate(template, title, translation) {
    ...
    ["titleI18n", "descriptionI18n", "snippetI18n"].forEach(key => {
        ...
        const val = getProp.getProp(translation, `addons.services.teams.groups.${i18nKey}`);
...
function doesGroupExist(title, hubRequestOptions) {
    ...
    return search.searchGroups(searchOpts)
...
function getAllowedGroupAccess(requestedAccess, user, portal) {
    ...
    const portalWideCanSharePublic = getProp.getProp(portal, "canSharePublic") || false;
...
function createHubTeams(opts) {
    ...
    const product = getSurveyModels.getHubProduct(hubRequestOptions.portalSelf);
    // get all the groups that this user can create in this environment
    // and filter just the team types requested
    const subscriptionType = getSurveyModels.getSubscriptionType(hubRequestOptions.portalSelf);
    ...
    const culture = getSurveyModels.getCulture(hubRequestOptions);
    const locale = getSurveyModels.convertToWellKnownLocale(culture);
    // Fire that off
    return getSurveyModels.fetchHubTranslation(locale, hubRequestOptions.portalSelf)
        .then((translations) => {
...
function isSite(item) {
    return compose.normalizeItemType(item) === "Hub Site Application";
}
...
function removeSiteFromPage(siteId, pageModel) {
    ...
    return getWithDefault.getWithDefault(pageModel, "data.values.sites", []).filter((e) => {
        return e.id !== siteId;
    });
}
...
function unlinkSiteAndPage(unlinkRequestOptions) {
    ...
    return Promise.all([
        getSurveyModels.getModelFromOptions("page", unlinkRequestOptions),
        getSurveyModels.getModelFromOptions("site", unlinkRequestOptions)
    ])
        .then(models => {
        [pageModel, siteModel] = models;
        // Handle the site
        if (!siteModel.isMissing) {
            const pages = getWithDefault.getWithDefault(siteModel, "data.values.pages", []);
            // remove the page from the pages array on the model
            siteModel.data.values.pages = getSurveyModels.withoutByProp("id", pageModel.item.id, pages);
            // collect the groups we'll unshare the page from
            unshareGroups = util.maybePush(getProp.getProp(siteModel, "item.properties.collaborationGroupId"), unshareGroups);
            unshareGroups = util.maybePush(getProp.getProp(siteModel, "item.properties.contentGroupId"), unshareGroups);
            // update the site, but failSafe so we don't have to do any checking if the current user can update it
            promises.push(getSurveyModels.failSafeUpdate(siteModel, requestOptions));
        }
        // Handle the page
        if (!pageModel.isMissing) {
            const sites = getWithDefault.getWithDefault(pageModel, "data.values.sites", []);
            // remove site from sites array on the model
            pageModel.data.values.sites = getSurveyModels.withoutByProp("id", siteModel.item.id, sites);
            promises.push(getSurveyModels.failSafeUpdate(pageModel, requestOptions));
            // now about the groups
            const failSafeUnshare = failSafe.failSafe(unshareItemFromGroups.unshareItemFromGroups);
...
function removePage(idOrModel, requestOptions) {
    ...
    if (typeof idOrModel === "string") {
        modelPromise = themes.getModel(idOrModel, requestOptions);
    }
    let pageModel;
    ...
    return modelPromise
        ...
        const linkedSites = mapBy.mapBy("id", getWithDefault.getWithDefault(pageModel, "data.values.sites", []));
        ...
        const failSafeUnlink = failSafe.failSafe(unlinkSiteAndPage);
        ...
        .then(() => {
        return getSurveyModels.unprotectModel(pageModel, requestOptions);
    })
        .then(() => {
        const opts = Object.assign({ id: pageModel.item.id }, requestOptions);
        return remove$1.removeItem(opts);
    });
}
...
function updatePage(model, updateSiteOptions) {
    ...
    if (patchList.length) {
        prms = themes.getModel(getProp.getProp(model, "item.id"), updateSiteOptions);
    }
    return prms.then(modelFromAGO => {
        if (patchList.length) {
            // "patch" operation
            model = mergeObjects.mergeObjects(model, modelFromAGO, patchList);
        }
        // update it
        const opts = Object.assign({ item: getSurveyModels.serializeModel(model) }, updateSiteOptions);
        opts.params = { clearEmptyFields: true };
        return update.updateItem(opts);
    });
}
...
function getCardDependencies(card) {
    const componentName = getProp.getProp(card, "component.name");
...
function collectAndFlattenPropertyValues(card) {
    return (acc, path) => {
        const propertyValue = getProp.getProp(card, path);
        if (!propertyValue) {
            return acc;
        }
...
function getPageDependencies(model) {
    const layout = getWithDefault.getWithDefault(model, "data.values.layout", {});
...
function ensureRequiredPageProperties(pageModel, options) {
    // clone
    const result = util.cloneObject(pageModel);
...
function linkSiteAndPage(linkRequestOptions) {
    ...
    // get the models from the options...
    return Promise.all([
        getSurveyModels.getModelFromOptions("page", linkRequestOptions),
        getSurveyModels.getModelFromOptions("site", linkRequestOptions),
    ])
        .then((models) => {
        ...
        if (!siteModel.isMissing && !pageModel.isMissing) {
            ...
            if (!isSite(siteModel.item) ||
                !compose.includes(["Hub Page", "Site Page"], pageModel.item.type)) {
            ...
            if (!getProp.getProp(pageModel, "data.values.sites")) {
                deepSet.deepSet(pageModel, "data.values.sites", []);
            }
            const sites = getProp.getProp(pageModel, "data.values.sites");
            const hasSiteAlready = compose.includes(sites.map((p) => p.id), siteEntry.id);
            if (!hasSiteAlready) {
                pageModel.data.values.sites.push(siteEntry);
                const opts = Object.assign({ item: getSurveyModels.serializeModel(pageModel) }, requestOptions);
                // Not failsafe - could reject
                promises.push(update.updateItem(opts));
            }
            ...
            if (!getProp.getProp(siteModel, "data.values.pages")) {
                deepSet.deepSet(siteModel, "data.values.pages", []);
            }
            const pages = getProp.getProp(siteModel, "data.values.pages");
            const hasPageAlready = compose.includes(pages.map((p) => p.id), pageEntry.id);
            if (!hasPageAlready) {
                const slugs = mapBy.mapBy("slug", pages);
                // use the passed in slug, or generate a unique slug and add to the page entry...
                pageEntry.slug =
                    linkRequestOptions.pageSlug ||
                        getSurveyModels.ensureUniqueString(slugs, slugify.slugify(pageEntry.title));
                // push entry into pages array...
                siteModel.data.values.pages.push(pageEntry);
                // update the site item...
                const opts = Object.assign({ item: getSurveyModels.serializeModel(siteModel) }, requestOptions);
                // Not failsafe - could reject
                promises.push(update.updateItem(opts));
            }
            ...
            shareGroups = util.maybePush(getProp.getProp(siteModel, "item.properties.collaborationGroupId"), shareGroups);
            shareGroups = util.maybePush(getProp.getProp(siteModel, "item.properties.contentGroupId"), shareGroups);
            // NOTE: Since sharing is limited to the owner || admin we failSafe the calls, and hope for the best.
            const failSafeShare = failSafe.failSafe(shareItemToGroups.shareItemToGroups);
...
function createPage(model, options, hubRequestOptions) {
    ...
    const serializedModel = getSurveyModels.serializeModel(newPage);
    // create the item
    return create.createItem({
        ...
        if (Array.isArray(options.shareTo) && options.shareTo.length) {
            ...
            sharingPromises = options.shareTo.map((groupInfo) => {
                return shareItemWithGroup.shareItemWithGroup({
                    id: newPage.item.id,
        ...
        .then(response => {
        // link page to sites
        const sites = getWithDefault.getWithDefault(newPage, "data.values.sites", []);
        const requestOptions = {
            authentication: hubRequestOptions.authentication
        };
        ...
        // upload resources
        const assets = getWithDefault.getWithDefault(options, "assets", []);
        return getSurveyModels.uploadResourcesFromUrl(newPage, assets, hubRequestOptions);
    })
        .then(() => newPage)
        .catch(err => {
...
function _getImageCropIdsFromLayout(layout) {
    const imgAssets = [];
    const headerLogo = getProp.getProp(layout, "header.component.settings.logo");
    if (headerLogo && headerLogo.cropId) {
        imgAssets.push(headerLogo);
    }
    const sections = getProp.getProp(layout, "sections") || [];
    return sections
...
function collectSectionAssets(assets, section) {
    ...
    if (getProp.getProp(section, "style.background.cropSrc")) {
...
function convertCard(card) {
    const clone = util.cloneObject(card);
...
function convertImageCard(card) {
    ...
    if (getProp.getProp(card, "component.settings.fileSrc")) {
        result.assets.push(card.component.settings.fileSrc);
    }
    if (getProp.getProp(card, "component.settings.cropSrc")) {
...
function convertItemGalleryCard(card) {
    ...
    const version = getProp.getProp(settings, "version");
    if (getProp.getProp(settings, "groups") && version < 4) {
        settings.groups = [
            {
    ...
    if (getProp.getProp(settings, "query.groups")) {
        if (version >= 4) {
    ...
    if (getProp.getProp(settings, "query.orgId")) {
        settings.query.orgId = "{{organization.id}}";
    }
    if (getProp.getProp(settings, "orgId")) {
...
function convertSection(section) {
    ...
    const result = {
        section: util.cloneObject(section),
        assets
    };
...
function sectionHasBackgroundFile(clonedSection) {
    return getProp.getProp(clonedSection, "style.background.fileSrc");
}
...
function convertLayoutToTemplate(layout) {
    ...
    if (layout.header) {
        result.layout.header = util.cloneObject(layout.header);
    }
    if (layout.footer) {
        result.layout.footer = util.cloneObject(layout.footer);
    }
    return result;
}
...
function removeUnusedResources(id, layout, hubRequestOptions) {
    ...
    return get.getItemResources(id, hubRequestOptions).then(response => {
...
function removeUnusedResourcesFromAGO(id, unusedCrops, authentication) {
    ...
    const failSaveRemoveItemResources = failSafe.failSafe(remove$1.removeItemResource, {
        success: true
    });
...
function getSiteDependencies(model) {
    const pages = getProp.getProp(model, "data.values.pages") || [];
    const pageIds = pages.map((p) => p.id);
    const layout = getProp.getProp(model, "data.values.layout") || {};
...
function convertPageToTemplate(model, hubRequestOptions) {
    const tmpl = util.cloneObject(model);
    // set things we always want...
    tmpl.type = getPageItemType(hubRequestOptions.isPortal);
    tmpl.key = `${getSurveyModels.propifyString(model.item.title)}_${util.createId("i")}`;
    tmpl.itemId = model.item.id;
    // now pass the item off to be normalized
    tmpl.item = getSurveyModels.normalizeSolutionTemplateItem(tmpl.item);
    ...
    tmpl.data = getSurveyModels.replaceItemId(tmpl.data, tmpl.itemId);
    if (!tmpl.item.properties) {
        tmpl.item.properties = {};
    }
    return getSurveyModels.getItemAssets(model.item, hubRequestOptions).then((assets) => {
...
function createPageModelFromTemplate(template, settings, transforms, hubRequestOptions) {
    ...
    template.assets = getSurveyModels.addSolutionResourceUrlToAssets(template, hubRequestOptions);
    ...
    const pageModel = interpolate.interpolate(template, settings, transforms);
    ...
    const parentInitiativeId = getProp.getProp(settings, "initiative.id");
    if (parentInitiativeId) {
        deepSet.deepSet(pageModel, "item.properties.parentInitiativeId", parentInitiativeId);
    }
    // put the slug into the hash so we can use it in following templates
    deepSet.deepSet(pageModel, "data.values.slug", slugify.slugify(pageModel.item.title));
...
function ensurePageHasSiteEntry(siteModel, pageModel) {
    ...
    const parentInitiativeId = getProp.getProp(siteModel, "item.properties.parentInitiativeId");
    // swap initiativeId to siteId
    // for a period of time, this happened during Solution generation
    if (parentInitiativeId) {
        const currentSites = getProp.getProp(pageModel, "data.values.sites");
...
function isPage(item) {
    return compose.normalizeItemType(item) === "Hub Page";
}
...
function hasUnpublishedChanges(siteOrPageModel) {
    return compose.includes(siteOrPageModel.item.typeKeywords, UNPUBLISHED_CHANGES_KW);
}
...
function markPublished(siteOrPageModel) {
    const model = util.cloneObject(siteOrPageModel);
...
function markUnpublished(siteOrPageModel) {
    const model = util.cloneObject(siteOrPageModel);
    if (!compose.includes(model.item.typeKeywords, UNPUBLISHED_CHANGES_KW)) {
...
function _ensurePortalDomainKeyword(subdomain, typeKeywords = []) {
    ...
    if (compose.includes(typeKeywords, expectedKeyword)) {
        return typeKeywords;
    }
    else {
...
function updateSite(model, updateSiteOptions) {
    ...
    deepSet.deepSet(model, "data.values.uiVersion", SITE_UI_VERSION);
    deepSet.deepSet(model, "data.values.updatedAt", new Date().toISOString());
    deepSet.deepSet(model, "data.values.updatedBy", updateSiteOptions.authentication.username);
    ...
    if (updateSiteOptions.isPortal) {
        model.item.typeKeywords = _ensurePortalDomainKeyword(getProp.getProp(model, "data.values.subdomain"), model.item.typeKeywords);
    ...
    if (allowList.length) {
        agoModelPromise = themes.getSiteById(model.item.id, updateSiteOptions);
    }
    else {
    ...
    return agoModelPromise
        ...
        if (allowList.length) {
            ...
            model = mergeObjects.mergeObjects(model, agoModel, allowList);
        }
        // send the update to ago
        return update.updateItem({
            item: getSurveyModels.serializeModel(model),
...
function _getDraftResourceNames(siteOrPageId, hubRequestOptions) {
    return get.getItemResources(siteOrPageId, {
...
function deleteDraft(siteOrPageModel, hubRequestOptions) {
    ...
    return _getDraftResourceNames(id, hubRequestOptions).then(draftResourceNames => Promise.all(draftResourceNames.map(resourceName => remove$1.removeItemResource({
        id,
        owner,
...
function saveDraft(siteOrPageModel, hubRequestOptions) {
    ...
    const draft = getSurveyModels.buildDraft(siteOrPageModel, includeList);
    const draftBlob = objectToJsonBlob.objectToJsonBlob(draft);
    const draftName = `draft-${Date.now()}.json`;
    const itemId = getProp.getProp(siteOrPageModel, "item.id");
    return deleteDraft(siteOrPageModel, hubRequestOptions)
        .then((_) => themes.addItemResource({
        id: itemId,
        owner: getProp.getProp(siteOrPageModel, "item.owner"),
        resource: draftBlob,
        name: draftName,
        private: false,
        portal: hubRequestOptions.portal,
...
function upgradeDraftSchema(draft) {
    if (getProp.getProp(draft, "item.properties.schemaVersion") === undefined) {
        deepSet.deepSet(draft, schemaVersionPath, initialDraftVersion);
    }
    // Migrations that should always be applied
    draft = themes.migrateBadBasemap(draft);
    draft = themes.migrateWebMappingApplicationSites(draft);
    if (getProp.getProp(draft, "item.properties.schemaVersion") === themes.SITE_SCHEMA_VERSION) {
        return draft;
    }
    else {
        let migrated = draft;
        ...
        // at version 1.3
        migrated = themes._ensureTelemetry(draft);
        migrated = themes._migrateFeedConfig(draft);
        migrated = themes._migrateEventListCardConfigs(draft);
        migrated = themes._migrateTelemetryConfig(draft);
        return migrated;
    }
}
...
function isSiteDraft(draft) {
    ...
    return (getProp.getProp(draft, "data.values.capabilities") !== undefined &&
        getProp.getProp(draft, "data.values.theme") !== undefined);
}
/**
...
function fetchDraft(siteOrPageId, hubRequestOptions) {
    return _getMostRecentDraftName(siteOrPageId, hubRequestOptions)
        ...
        if (draftResourceName) {
            ret = get.getItemResource(siteOrPageId, {
...
function applyDraft(siteOrPageModel, draft) {
    ...
    const includeList = _includeListFromItemType(siteOrPageModel.item);
    return mergeObjects.mergeObjects(draft, util.cloneObject(siteOrPageModel), includeList);
}
...
function fetchAndApplyDraft(siteOrPageModel, hubRequestOptions) {
    return fetchDraft(getProp.getProp(siteOrPageModel, "item.id"), hubRequestOptions).then(draft => applyDraft(siteOrPageModel, draft));
}
...
function getDomains(siteId, requestOptions) {
    const url = domainExists._getDomainServiceUrl(compose.getHubApiUrl(requestOptions));
...
function _removeSiteGroups(siteModel, requestOptions) {
    const teamsToDelete = [
        ...
        return util.maybePush(getProp.getProp(siteModel, `item.properties.${prop}`), acc);
    }, []);
    const promises = teamsToDelete.map(id => {
        const opts = Object.assign({ id }, requestOptions);
        return getSurveyModels._unprotectAndRemoveGroup(opts);
    });
    return Promise.all(promises);
}
...
function _removeParentInitiative(siteModel, requestOptions) {
    ...
    if (parentInitiativeId) {
        ...
        return getSurveyModels._unprotectAndRemoveItem(opts);
    }
    else {
        return Promise.resolve();
    }
...
function _removeSiteFromIndex(siteModel, hubRequestOptions) {
    if (hubRequestOptions.isPortal) {
        ...
        const url = `${compose.getHubApiUrl(hubRequestOptions)}/api/v3/${siteModel.item.id}`;
        const opts = {
            method: "DELETE",
            mode: "cors",
            headers: {
...
function _removeSiteDomains(siteId, hubRequestOptions) {
    ...
    return themes.removeDomainsBySiteId(siteId, hubRequestOptions).then((response) => response);
}
...
function _ensureOptionalGroupsTemplating(itemTemplate) {
    ...
    const tmpl = util.cloneObject(itemTemplate);
    props.forEach(prop => {
        const val = getProp.getProp(tmpl, `properties.${prop}`);
        if (val) {
            if (val.match(/^\{\{.*\}\}$/)) {
...
function _ensureTypeAndTags(model, isPortal) {
    ...
    if (!compose.includes(model.item.typeKeywords, "hubSite")) {
...
function updateSiteApplicationUris(site, uris, hubRequestOptions) {
    ...
    return getSurveyModels.getDomainsForSite(site.item.id, hubRequestOptions).then((domainInfos) => {
        // get all domains that are no longer associated with the site
        const domainsToRemove = domainInfos.filter((domain) => !compose.includes(uris, domain.hostname));
        // get all new domains that are now associated with the site
        const hostnames = domainInfos.map((domain) => domain.hostname);
        const domainsToAdd = uris.filter((uri) => !compose.includes(hostnames, uri));
        // finally, kick all the promises
        const domainPromises = [];
        domainsToRemove.forEach((domain) => domainPromises.push(themes.removeDomain(domain.id, hubRequestOptions)));
        domainsToAdd.forEach((uri) => domainPromises.push(themes.addDomain({
...
function updateAppRedirectUris(clientId, redirectUris, requestOptions) {
    const url = `${getPortalApiUrl.getPortalApiUrl(requestOptions)}/oauth2/apps/${clientId}/update`;
    const options = {
        method: "POST",
...
function unlinkPagesFromSite(siteModel, requestOptions) {
    const linkedPages = mapBy.mapBy("id", getWithDefault.getWithDefault(siteModel, "data.values.pages", []));
...
function removeSite(idOrModel, hubRequestOptions) {
    let modelPromise;
    if (typeof idOrModel === "string") {
        modelPromise = themes.getModel(idOrModel, hubRequestOptions);
    }
    else {
    ...
    return modelPromise
        ...
        return failSafe.failSafe(_removeSiteFromIndex, { success: true })(siteModel, hubRequestOptions);
    })
        .then(() => {
...
function ensureRequiredSiteProperties(model, username, isPortal = false) {
    ...
    if (!model.item.url) {
        const hostname = getProp.getProp(model, "data.values.customHostname") ||
            getProp.getProp(model, "data.values.defaultHostname");
    ...
    if (!Array.isArray(getProp.getProp(model, "data.values.pages"))) {
        deepSet.deepSet(model, "data.values.pages", []);
    }
    deepSet.deepSet(model, "data.values.uiVersion", SITE_UI_VERSION);
    ...
    deepSet.deepSet(model, "data.values.capabilities", caps);
    if (!getProp.getProp(model, "data.telemetry")) {
        deepSet.deepSet(model, "data.telemetry", {});
    }
    // return the clone
    return model;
}
...
function saveModel(model, requestOptions) {
    ...
    return create.createItem(opts).then((response) => {
...
function getDefaultInitiativeTemplate(hubRequestOptions) {
    const culture = getSurveyModels.getCulture(hubRequestOptions);
    const locale = getSurveyModels.convertToWellKnownLocale(culture);
    return getSurveyModels.fetchHubTranslation(locale, hubRequestOptions.portalSelf).then(translation => {
        // now we can get the embedded initiative template
        const tmpl = util.cloneObject(DEFAULT_INITIATIVE_TEMPLATE);
        // pluck values off the translation, and inject into the tmpl
        tmpl.item.description = getProp.getProp(translation, "addons.services.templates.customInitiative.item.description");
        tmpl.item.snippet = getProp.getProp(translation, "addons.services.templates.customInitiative.item.snippet");
...
function getInitiativeTemplate(siteTemplate, hubRequestOptions) {
    let tmplPromise;
    const template = getProp.getProp(siteTemplate, "properties.initiativeTemplate");
    if (template) {
...
function updateInitiativeSiteId(maybeModel, siteId, hubRequestOptions) {
    ...
    if (typeof maybeModel === "string") {
        ...
        if (getProp.getProp(maybeModel, "item.id")) {
            const m = maybeModel;
...
function createSite(model, options, hubRequestOptions) {
    ...
    return create.createItem({
        ...
        return themes.addSiteDomains(model, hubRequestOptions);
    })
        ...
        const dcatConfig = util.cloneObject(model.data.values.dcatConfig);
        delete model.data.values.dcatConfig;
        // with the id of the actual item
        model = getSurveyModels.interpolateItemId(model);
        ...
        return getSurveyModels.uploadResourcesFromUrl(model, options.assets || [], hubRequestOptions);
    })
        ...
        const collabGroupId = getProp.getProp(model, "item.properties.collaborationGroupId");
        if (collabGroupId) {
            sharePrms = shareItemWithGroup.shareItemWithGroup({
                id: model.item.id,
        ...
        const initiativeItemId = getProp.getProp(model, "item.properties.parentInitiativeId");
        if (initiativeItemId) {
...
function _createSiteInitiative(siteTemplate, settings, transforms, hubRequestOptions) {
    ...
    return getInitiativeTemplate(siteTemplate, hubRequestOptions)
        ...
        initiativeTemplate.item.typeKeywords = util.without(initiativeTemplate.item.typeKeywords, "Hub Initiative Template");
        ...
        const initiativeModel = interpolate.interpolate(initiativeTemplate, settings, transforms);
        // and save it
        ...
        let sharePrms = Promise.resolve({ success: true });
        // share it to the collab team if that got created
        const collabGroupId = getProp.getProp(settings, "teams.collaborationGroupId");
        if (collabGroupId) {
            sharePrms = shareItemWithGroup.shareItemWithGroup({
                id: model.item.id,
                groupId: collabGroupId,
...
function _updateTeamTags(initiativeModel, teams, hubRequestOptions) {
    ...
    /* istanbul ignore if */
    if (getProp.getProp(teams, "props.followersGroupId")) {
...
function interpolateSite(siteTemplate, settings, transforms) {
    const template = util.cloneObject(siteTemplate);
    // Save nested adlib templates
    const legacyDcatUS11Config = util.cloneObject(template.data.values.dcatConfig);
    const feedConfigs = util.cloneObject(template.data.feeds);
    ...
    const siteModel = interpolate.interpolate(template, settings, transforms);
...
function createSiteModelFromTemplate(template, settings, transforms, hubRequestOptions) {
    ...
    getSurveyModels.getHubProduct(hubRequestOptions.portalSelf);
    let title = getProp.getProp(settings, "solution.title") || "New Site";
    // handle issue with titles that are just numbers
    if (typeof title === "number") {
        title = title.toString();
        deepSet.deepSet(settings, "solution.title", title);
    }
    ...
    return createHubTeams({
        ...
        settings.teams = util.cloneObject(teams.props);
        ...
        if (getProp.getProp(teams, "props.contentGroupId")) {
            deepSet.deepSet(template, "data.catalog.groups", [teams.props.contentGroupId]);
        }
        ...
        return themes.ensureUniqueDomainName(slugify.slugify(domainTitle), hubRequestOptions);
    })
        ...
        settings.solution.theme = themes.getOrgDefaultTheme(portal);
        ...
        if (hubRequestOptions.isPortal) {
            ...
            const base = domainExists.stripProtocol(compose.getHubApiUrl(hubRequestOptions));
        ...
        if (typeof siteModel.item.title === "number") {
            siteModel.item.title = getProp.getProp(settings, "solution.title");
            siteModel.data.values.title = getProp.getProp(settings, "solution.title");
        }
        return siteModel;
    })
        .catch((ex) => {
...
function convertSiteToTemplate(model, hubRequestOptions) {
    ...
    tmpl.item.properties = {
        schemaVersion: themes.SITE_SCHEMA_VERSION,
        children: [],
    };
    ...
    if (getProp.getProp(tmpl, "data.values.defaultExtent")) {
        tmpl.data.values.defaultExtent = "{{organization.defaultExtent}}";
    }
    if (getProp.getProp(tmpl, "data.values.map.basemaps.primary.extent")) {
...
function _getSiteDataById(id, hubRequestOptions) {
    ...
    return get.getItemData(id, hubRequestOptions).then(data => {
...
function _getSecondPassSharingOptions(siteModel) {
    return [
        ...
        const groupId = getProp.getProp(siteModel, entry.prop);
        if (groupId) {
            acc.push({
                id: groupId,
...
function _getIneligibleModelIds(siteModel, models, hubRequestOptions) {
    const fetchSurveyModels = (model) => getSurveyModels.getSurveyModels(model.item, hubRequestOptions).then(({ form, featureService, fieldworker, stakeholder }) => [form, featureService, fieldworker, stakeholder].filter(Boolean));
...
function shareItemsToSiteGroups(siteModel, solutionModels, hubRequestOptions) {
    ...
    const failSafeShare = failSafe.failSafe(shareItemWithGroup.shareItemWithGroup, { success: true });
...
function _secondPassAdlibPages(siteModel, pageModel) {
    ...
    const teams = [
        ...
        const teamId = getProp.getProp(siteModel, `item.properties.${prop}`);
        if (teamId) {
            acc[prop] = teamId;
        }
        return acc;
    ...
    const settings = {
        teams,
        siteId: getProp.getProp(siteModel, "item.id"),
        siteUrl: getProp.getProp(siteModel, "item.url"),
        initiative: {
            item: { id: getProp.getProp(siteModel, "item.properties.parentInitiativeId") }
        }
    };
    // weld in the site itself so it can be used for some interpolations
    settings[siteModel.key] = siteModel;
    return interpolate.interpolate(pageModel, settings);
}
...
function _updatePages(siteModel, solutionModels, hubRequestOptions) {
    ...
    const failSafeUpdate = failSafe.failSafe(update.updateItem, { success: true });
    ...
    return Promise.all(pageModels.map(m => {
        ...
        return failSafeUpdate({
            item: getSurveyModels.serializeModel(m),
            authentication: hubRequestOptions.authentication
        });
    }));
}
...
function getDataForSiteItem(item, hubRequestOptions) {
    return get.getItemData(item.id, hubRequestOptions).then((data) => {
        return themes.upgradeSiteSchema({
            item,
            data,
        });
    });
...
function authenticatedGetMembers(usernames, requestOptions) {
    const urlPath = `${getPortalUrl.getPortalUrl(requestOptions)}/sharing/rest/community/users`;
    const chunkSize = 100;
    ...
    return batch.batch(chunkedOptions, batchMemberRequest).then((batchedMembers) => {
...
function unauthenticatedGetMembers(usernames, requestOptions) {
    return Promise.all(usernames.map((username) => {
        return getUser.getUser(Object.assign({ username }, requestOptions))
            .then((response) => {
            ...
            .catch((e) => {
            logger.Logger.error(`Error fetching user, ${username}, from AGO user endpoint, ${e}`);
            return null;
        });
...
function batchMemberRequest(options) {
    return request.request(options.urlPath, options.requestOptions)
        .then((response) => response.results)
        .catch((e) => {
        logger.Logger.error(`Error fetching members from AGO user search endpoint: ${e}`);
        return [];
    });
}
