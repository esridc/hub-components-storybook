...
const schedulerCss = ".max-w-21{max-width:21rem}.expanded-daily-scheduler-section,.expanded-weekly-scheduler-section,.expanded-monthly-scheduler-section,.expanded-yearly-scheduler-section{display:flex;flex-direction:column}.select-format{display:flex;flex-direction:column;gap:1rem}.expanded-sub-input{padding-bottom:0.5rem}";
...
const Scheduler = class {
  ...
  async componentWillLoad() {
    ...
    this.options = ((_b = (_a = this.params) === null || _a === void 0 ? void 0 : _a.uiSchema) === null || _b === void 0 ? void 0 : _b.options.inputs)
      ? util.cloneObject((_d = (_c = this.params) === null || _c === void 0 ? void 0 : _c.uiSchema) === null || _d === void 0 ? void 0 : _d.options.inputs)
  ...
  handleParamsChange(newParams, oldParams) {
    ...
    const isDisabled = getProp.getProp(newParams, 'uiSchema.options.disabled') || newParams.disabled;
    const wasDisabled = getProp.getProp(oldParams, 'uiSchema.options.disabled') || oldParams.disabled;
    ...
    if (!wasDisabled && isDisabled) {
      ...
      this._valueBeforeDisabled = util.cloneObject(this.value);
      this._optionsBeforeDisabled = util.cloneObject(this.options);
      ...
      this.options = util.cloneObject(this._optionsBeforeDisabled);
      this.value = util.cloneObject(this._valueBeforeDisabled);
  ...
  updateHour(e) {
    ...
    const hoursAndMinutes = util.cloneObject(timePicker.value.split(":"));
  ...
  renderInvalidInputMessage(message) {
    return index.h("calcite-input-message", { icon: "x-octagon", status: "invalid" }, message);
  }
  renderHourPicker() {
    var _a;
    return index.h("calcite-label", { class: "expanded-sub-input" }, this.intl.t('inputs.hour.helperText'), index.h("calcite-input-time-picker", { class: "max-w-21", id: "scheduler-time-picker", placement: "bottom", status: this.isHourValid ? "idle" : "invalid", value: this.isHourValid ? `${(_a = this.value) === null || _a === void 0 ? void 0 : _a.hour}:00` : null }), this.isHourValid
      ? index.h("calcite-input-message", { icon: "information-f", status: "idle" }, this.intl.t('inputs.hour.inputMessage'))
      : this.renderInvalidInputMessage(this.intl.t('inputs.hour.error')));
  }
  renderDatePicker() {
    var _a;
    return index.h("calcite-label", { class: "expanded-sub-input" }, this.intl.t('inputs.date.helperText'), index.h("calcite-input-number", { class: "max-w-21", clearable: true, integer: true, max: "28", min: "1", "number-button-type": "vertical", placeholder: this.intl.t('inputs.date.inputNumberPlaceholder'), status: this.isDateValid ? "idle" : "invalid", step: "1", value: ((_a = this.value) === null || _a === void 0 ? void 0 : _a.date) || null }), this.isDateValid
      ? index.h("calcite-input-message", { icon: "information-f", status: "idle" }, this.intl.t('inputs.date.inputMessage'))
  ...
  renderDayPicker() {
    ...
    return index.h("calcite-label", { class: "expanded-sub-input" }, this.intl.t('inputs.day.helperText'), index.h("calcite-select", { class: "max-w-21", id: "day-picker", label: this.intl.t('inputs.day.selectLabel') }, daysOfTheWeek.map(day => {
      var _a;
      const selected = `${(_a = this.value) === null || _a === void 0 ? void 0 : _a.day}` === day.value;
      return index.h("calcite-option", { key: day.value, label: day.label, selected: selected, status: this.isDayValid ? "idle" : "invalid", value: day.value }, day.label);
  ...
  renderMonthPicker() {
    ...
    return index.h("calcite-label", { class: "expanded-sub-input" }, this.intl.t('inputs.month.helperText'), index.h("calcite-select", { class: "max-w-21", id: "month-picker", label: this.intl.t('inputs.month.selectLabel') }, monthsOfTheYear.map(month => {
      var _a;
      const selected = `${(_a = this.value) === null || _a === void 0 ? void 0 : _a.month}` === month.value;
      return index.h("calcite-option", { key: month.value, label: month.label, selected: selected, status: this.isValidMonth ? "idle" : "invalid", value: month.value }, month.label);
    })), !this.isValidMonth && this.renderInvalidInputMessage(this.intl.t('inputs.month.error')));
  }
  renderTimezonePicker() {
    var _a;
    return index.h("calcite-label", { class: "expanded-sub-input" }, this.intl.t('inputs.timezone.helperText'), index.h("calcite-input-time-zone", { class: "max-w-21", mode: "name", status: this.isTimezoneValid ? "idle" : "invalid", value: (_a = this.value) === null || _a === void 0 ? void 0 : _a.timezone }), !this.isTimezoneValid && this.renderInvalidInputMessage(this.intl.t('inputs.timezone.error')));
  }
  /*********** RENDER FUNCTIONS: EXPANDED SECTIONS ***********/
  renderDailyOptionSection() {
    return index.h("div", { class: "expanded-daily-scheduler-section" }, this.renderHourPicker(), this.renderTimezonePicker());
  }
  renderWeeklyOptionSection() {
    return index.h("div", { class: "expanded-weekly-scheduler-section" }, this.renderDayPicker(), this.renderHourPicker(), this.renderTimezonePicker());
  }
  renderMonthlyOptionSection() {
    return index.h("div", { class: "expanded-monthly-scheduler-section" }, this.renderDatePicker(), this.renderHourPicker(), this.renderTimezonePicker());
  }
  renderYearlyOptionSection() {
    return index.h("div", { class: "expanded-yearly-scheduler-section" }, this.renderMonthPicker(), this.renderDatePicker(), this.renderHourPicker(), this.renderTimezonePicker());
  }
  ...
  renderExpandedOptionSection() {
    ...
    switch (type) {
      case 'daily':
        return index.h("div", null, this.renderDailyOptionSection());
      case 'weekly':
        return index.h("div", null, this.renderWeeklyOptionSection());
      case 'monthly':
        return index.h("div", null, this.renderMonthlyOptionSection());
      case 'yearly':
        return index.h("div", null, this.renderYearlyOptionSection());
    }
  }
  /*********** RENDER FUNCTIONS: FORMATS ***********/
  renderSelectOption(option) {
    const { type, checked } = option;
    return index.h("calcite-option", { selected: checked, value: type }, this.getOptionLabel(option));
  }
  renderSelectFormat() {
    var _a, _b, _c, _d;
    const labelKey = (_b = (_a = this.params) === null || _a === void 0 ? void 0 : _a.uiSchema) === null || _b === void 0 ? void 0 : _b.options.labelKey;
    const select = index.h("calcite-select", { class: "max-w-21", disabled: this._disabled, id: "scheduler-select", label: (_c = this.params) === null || _c === void 0 ? void 0 : _c.label, name: (_d = this.params) === null || _d === void 0 ? void 0 : _d.label, required: true, width: "full" }, this.options.map(option => {
      var _a;
    ...
    return index.h("div", { class: "select-format" }, labelKey
      ? index.h("calcite-label", null, this.intl.t(labelKey), select)
      : select, this.renderExpandedOptionSection());
  }
  renderSingleFormat() {
    return index.h("div", { class: "single-format" }, this.renderExpandedOptionSection());
  }
  render() {
    var _a, _b;
    let scheduler;
    ...
    return index.h(index.Host, { "data-element": "scheduler-field" }, scheduler);
  }
  static get assetsDirs() { return ["locales"]; }
  get element() { return index.getElement(this); }
  static get watchers() { return {
    "params": ["handleParamsChange"],
    "value": ["handleArcgisHubInputSchedulerChange"]
  }; }
};
Scheduler.style = schedulerCss;

exports.hub_field_input_scheduler = Scheduler;
