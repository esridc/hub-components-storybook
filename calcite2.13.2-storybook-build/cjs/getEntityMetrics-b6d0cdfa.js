...
function getEntityMetrics(entity) {
    ...
    const metrics = entityMetrics.map((metric) => {
        ...
        if (source.type === "item-query" && source.collectionKey) {
            ...
            const query = getProp.getProp(entity.catalog, `collections[findBy(key,'${key}')].scope`);
            if (query) {
                source.scope = util.cloneObject(query);
            }
        }
...
exports.getEntityMetrics = getEntityMetrics;
