...
const getElementPath = (composedPath) => {
  ...
  const elementPath = composedPath.reduce((path, el) => {
    ...
    if (namedItem) {
      ...
      const value = dasherize.dasherize(namedItem.value).split('-')
        .map(word => util.capitalize(word)).join(' ');
...
const getTeamId = (object, type) => {
  return getProp.getProp(object, `${TYPEMAP[type]}`);
};
/**
...
exports.getEntityTelemetryDimensions = getEntityTelemetryDimensions;
