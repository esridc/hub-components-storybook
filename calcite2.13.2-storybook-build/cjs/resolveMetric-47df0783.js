 ...
 * @returns
 */
async function resolveMetric(metric, context) {
    ...
    switch (source.type) {
        case "static-value":
            return resolveStaticValueMetric(metric);
        case "service-query":
 ...
 * @returns
 */
function resolveStaticValueMetric(metric, context) {
    const source = metric.source;
 ...
 * @returns
 */
async function resolveServiceQueryMetric(metric, context) {
    const source = metric.source;
    // cut off the parent identifier from the metric id and use that
    // as the output field name
    const fieldName = metric.id.split("_")[0];
    ...
    // Execute the query
    const response = await fetchContent.queryFeatures({
        url: serviceUrl,
        where: source.where,
        f: "json",
    ...
    const aggregate = getProp.getProp(response, `features[0].attributes.${source.field}`);
    const result = {
        attributes: {
            id: metric.entityInfo.id,
            name: metric.entityInfo.name,
...
async function resolveItemQueryMetric(metric, context) {
    const source = metric.source;
    // cut off the parent identifier from the metric id and use that
    // as the output field name
    const fieldName = metric.id.split("_")[0];
    ...
    const combined = getTypeWithKeywordQuery.combineQueries([baseQuery, scope]);
    const opts = {
    ...
    const response = await HubInitiatives.portalSearchItemsAsItems(combined, opts);
    ...
    // resolved.
    const promises = await response.results.reduce(async (valsPromise, item) => {
        const vals = await valsPromise;
        const valueFromItem = getProp.getProp(item, source.propertyPath);
        const result = {
            attributes: {
                id: item.id,
                name: item.title,
        ...
        if (valueFromItem) {
            ...
            if (typeof valueFromItem === "string" ||
...
exports.resolveMetric = resolveMetric;
