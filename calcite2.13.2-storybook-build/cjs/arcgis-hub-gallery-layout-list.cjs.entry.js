...
const ArcgisHubGalleryLayoutList = class {
  constructor(hostRef) {
    ...
    this.gridColumns = 4;
    context.bind(this, 'handleResize', 'setPreviousResultsEnd');
  }
  get selectable() {
  ...
  renderResult(result, idx = 0) {
    ...
    this.showLinkButton && actionLinks.unshift({
      ...
      showLabel: true,
      href: getCardModelUrl.getCardModelUrlFromResult(result, this.linkTarget, this.baseUrl),
      buttonStyle: this.linkButtonStyle
    });
    return (index.h("arcgis-hub-entity-card", { actionLinks: actionLinks, baseUrl: this.baseUrl, callback: this.callback,
  ...
  _renderLoading() {
    ...
    const skeletons = [...Array(this.limit).keys()].map((idx) => {
      return index.h("arcgis-skeleton-loader", { active: true, headingRows: headingRows, key: idx, rows: 3, showHeading: true, showThumbnail: this.layout !== "list" }, this.showLinkButton && index.h("div", { class: "loading-link-button" }));
    });
    return index.h("div", { class: "card-container" }, skeletons);
  }
  /**
    ...
    if (!!searchResults.length) {
      const results = searchResults.reduce((acc, result, idx) => {
        acc.push(this.layout === 'list'
          ...
          ? index.h("li", null, this.renderResult(result, idx))
      ...
      result = index.h(index.Fragment, null, index.h(ContainerTag, { class: {
          'card-container': true,
  ...
  render() {
    return (index.h(index.Host, { "data-element": "gallery-layout-list" }, this.loading ? this._renderLoading() : this._renderResults()));
  }
...
exports.arcgis_hub_gallery_layout_list = ArcgisHubGalleryLayoutList;
