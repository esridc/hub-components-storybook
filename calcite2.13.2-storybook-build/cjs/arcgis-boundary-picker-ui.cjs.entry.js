...
const ArcgisBoundaryPickerUi = class {
  constructor(hostRef) {
    ...
    this.arcgisBoundaryPickerUpdate = index.createEvent(this, "arcgisBoundaryPickerUpdate", 7);
    this.arcgisBoundaryPickerCancel = index.createEvent(this, "arcgisBoundaryPickerCancel", 7);
    this.arcgisBoundaryPickerSave = index.createEvent(this, "arcgisBoundaryPickerSave", 7);
    ...
    context.bind(this, 'handleSave', 'handleCancel', 'handleEditClick', 'handleCancelEditClick', 'handleSaveEditClick', '_setMapDrawElement');
  }
  get _editing() {
  ...
  get _selectedGraphics() {
    ...
    if (graphic && !graphic.symbol && graphic.geometry && graphic.geometry.type) {
      graphic.symbol = map.getExtentSymbol();
    }
  ...
  get _saveGeometry() {
    return this._isGeometryDirty
      ...
      ? this._selectedValue === 'item'
        ...
        ? webMercatorUtils_js.webMercatorToGeographic(this._updatedGeometry.extent)
  ...
  handleTelemetry(e) {
    ...
    const actions = index$2.dist.dictionary.category.interaction.action;
    if (index$1.matchesTelemetry(detail, actions.search.label.filter) ||
      index$1.matchesTelemetry(detail, actions.enable.label.draw.details.update) ||
      index$1.matchesTelemetry(detail, actions.disable.label.draw.details.update)) {
      e.stopPropagation();
    }
  }
  ...
  renderEditConfirmButton() {
    if (this._editing) {
      return (index.h("calcite-action", { icon: 'check', onClick: this.handleSaveEditClick, slot: "actions-end", text: "" }));
    }
  }
  render() {
    var _a;
    return (index.h(index.Host, null, index.h("div", { class: "row" }, index.h("arcgis-hub-map", { basemap: "gray-vector", expand: EXPAND_FACTOR, extent: this.extent, graphics: this._selectedGraphics }, index.h("arcgis-hub-map-widget-container", { "expand-disabled": true, scale: "s", view: this._view, "view-position": "top-right" }, index.h("arcgis-hub-map-widget-zoom", { scale: "s", view: this._view })), index.h("arcgis-hub-map-widget-container", { scale: "s", "view-position": "top-right" }, index.h("arcgis-hub-map-widget-draw", { disableEditOptions: true, disablePrimaryOptions: true, geometry: this._editGeometry, ref: this._setMapDrawElement, resetOnDisconnect: this.resetDrawingToolsOnDisconnect, tools: [], view: this._view })), this._overlayMessage
      ? (index.h("div", { class: "arcgis-boundary-map-overlay" }, index.h("div", { class: "arcgis-boundary-none-message" }, this._overlayMessage)))
      : null), index.h("calcite-list", { "aria-busy": "false", "aria-disabled": this._isLoadingOrSaving, disabled: this._isLoadingOrSaving, role: "menu", selectionMode: "single-persist" }, this._noneOption
      ? index.h("calcite-list-item", { label: this._noneOption.label || this._getDefaultLabel(this._noneOption.value), selected: this._noneOption.selected, tabindex: "-1", value: this._noneOption.value })
      : null, index.h("calcite-list-group", { "group-title": this.intl.t('startFrom') }, (_a = this._otherOptions) === null || _a === void 0 ? void 0 : _a.map(option => index.h("calcite-list-item", { key: option.value, label: option.label || this._getDefaultLabel(option.value), selected: option.selected, value: option.value }, this.renderEditConfirmButton(), index.h("calcite-action", { disabled: this._selectedValue !== option.value, icon: this._editing ? 'x' : 'pencil', onClick: this._editing ? this.handleCancelEditClick : this.handleEditClick, slot: "actions-end", text: "" })))))), this.authenticated &&
      index.h("div", { class: "buttons" }, index.h("calcite-button", { appearance: "solid", color: "blue", disabled: !this._canSave || this.state === 'saving', loading: this.state === 'saving', onClick: this.handleSave, scale: "m", type: "button", width: "auto" }, this.intl.t('save')), index.h("calcite-button", { appearance: "outline", color: "blue", disabled: this._isLoadingOrSaving, onClick: this.handleCancel, scale: "m", type: "button", width: "auto" }, this.intl.t('cancel')))));
  }
...
exports.arcgis_boundary_picker_ui = ArcgisBoundaryPickerUi;
