 ...
 */
async function getWorkspaceLinks(entity, _context) {
  const entityType = getTypeFromEntity.getTypeFromEntity(entity);
  const entityWorkspaceLinkDefinitions = getWorkspaceLinkDefinitions.getWorkspaceLinkDefinitions(entityType);
...
function _checkWorkspaceLink(linkDefinition, entity, _context) {
  ...
  if (permissions === null || permissions === void 0 ? void 0 : permissions.length) {
    isAccessGranted = permissions.every((permission) => {
      const chk = checkPermission.checkPermission(permission, _context, entity);
      return chk.access;
    });
  }
...
exports.getWorkspaceLinks = getWorkspaceLinks;
