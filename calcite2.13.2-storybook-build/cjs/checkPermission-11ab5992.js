...
const HubPermissionsPolicies = [
    ...TemplateBusinessRules.SitesPermissionPolicies,
    ...TemplateBusinessRules.ProjectPermissionPolicies,
    ...TemplateBusinessRules.InitiativePermissionPolicies,
    ...DiscussionPermissionPolicies,
    ...ContentPermissionPolicies,
    ...GroupPermissionPolicies,
    ...TemplateBusinessRules.PagePermissionPolicies,
    ...TemplateBusinessRules.TemplatePermissionPolicies,
    ...PlatformPermissionPolicies,
    ...InitiativeTemplateBusinessRules.InitiativeTemplatePermissionPolicies,
...
const validPermissions = [
    ...TemplateBusinessRules.SitePermissions,
    ...TemplateBusinessRules.ProjectPermissions,
    ...TemplateBusinessRules.InitiativePermissions,
    ...ContentPermissions,
    ...GroupPermissions,
    ...TemplateBusinessRules.PagePermissions,
    ...PlatformPermissions,
    ...DiscussionPermissions,
    ...InitiativeTemplateBusinessRules.InitiativeTemplatePermissions,
    ...TemplateBusinessRules.TemplatePermissions,
    ...SystemPermissions,
    ...SurveyPermissions,
    ...EventPermissions,
    ...UserPermissions,
...
function checkEdit(policy, context, entity) {
    ...
    if (policy.hasOwnProperty("entityEdit")) {
        ...
        const check = {
            ...
            value: `entity.canEdit: ${getWithDefault.getWithDefault(entity, "canEdit", false)}`,
            code: getPolicyResponseCode(response),
            response,
        };
        checks.push(check);
...
function checkDelete(policy, context, entity) {
    ...
    if (policy.hasOwnProperty("entityDelete")) {
        ...
        const check = {
            ...
            value: `entity.canDelete: ${getWithDefault.getWithDefault(entity, "canDelete", false)}`,
            code: getPolicyResponseCode(response),
            response,
        };
        checks.push(check);
...
function checkEntityPolicy(policy, context) {
    ...
    if (type === "group-admin") {
        ...
        const memberType = getProp.getProp(group, "userMembership.memberType");
...
function checkAssertion(assertion, entity, context) {
    ...
    const propValue = getProp.getProp(lookupHash, propertyLookup.path);
    ...
    if (typeof val === "string" && val.indexOf(":") > -1) {
        ...
        val = getProp.getProp(lookupHash, valueLookup.path);
        if (val === undefined) {
            response = "assertion-property-not-found";
        }
    }
...
function groupAssertions(assertion, propValue, val, context) {
    ...
    let groups = mapBy.mapBy("id", userGroups);
...
function checkPermission(permission, context, entityOrOptions) {
    ...
    if ((_c = context.userHubSettings) === null || _c === void 0 ? void 0 : _c.preview) {
        const preview = getWithDefault.getWithDefault(context, "userHubSettings.preview", {});
        Object.keys(preview).forEach((key) => {
            ...
            if (permission === `hub:feature:${key}` &&
                getProp.getProp(preview, key) === true) {
                flagging.hasFlag = true;
                flagging.value = true;
                flagging.type = "feature";
            }
    ...
    if (entity) {
        const entityPolicies = getWithDefault.getWithDefault(entity, "permissions", []);
...
exports.HubPermissionsPolicies = HubPermissionsPolicies;
