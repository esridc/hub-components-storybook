...
const STAT_CARD_SCHEMA_VERSION = 1.0;
function migrateSummaryStatCardSettings(settings = {}, entity) {
  ...
  if (!settings.schemaVersion || settings.schemaVersion !== STAT_CARD_SCHEMA_VERSION) {
    ...
    if (!isNil.isNil(settings.type)) {
      ...
      settings = {
        ...
        cardId: settings.cardId || util.createId(),
...
function getItemIdAndLayerId(isDataset, settings) {
  ...
  if (isDataset) {
    const parsed = slugs.parseDatasetId(settings.datasetId);
  ...
  if (where) {
    ...
    expressionSet = whereClauses
      ...
      if (clause.includes(">=")) {
        ...
        expression = makeExpression(fieldName, fieldType, resources.ExpressionRelationships.BETWEEN, [finalValue]);
      }
      // could be number or date AND value is second in values array
        ...
        expression = makeExpression(fieldName, fieldType, resources.ExpressionRelationships.BETWEEN, [
          undefined,
          finalValue,
        ]);
      }
      // exact string match
        ...
        expression = makeExpression(fieldName, "esriFieldTypeString", resources.ExpressionRelationships.IS_EXACTLY, [
          cleanedValue,
        ]);
      }
      // deprecated "like" query that we used 2019 and prior
      else if (clause.includes(resources.ExpressionRelationships.LIKE)) {
        let { fieldName, value } = handleClauseSplit(clause, resources.ExpressionRelationships.LIKE);
        ...
        expression = makeExpression(fieldName, "esriFieldTypeString", resources.ExpressionRelationships.LIKE, [
          value
        ]);
      }
      return expression;
 ...
 * @returns
 */
function createDynamicSourceLink(linkBase, layerId) {
  // add layer id if we have it
  if (linkBase && !isNil.isNil(layerId)) {
...
exports.createDynamicSourceLink = createDynamicSourceLink;
exports.migrateSummaryStatCardSettings = migrateSummaryStatCardSettings;
