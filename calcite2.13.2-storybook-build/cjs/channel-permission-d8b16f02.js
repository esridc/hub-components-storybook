...
function dtoToChannel(dto) {
    const { channelAclDefinition } = dto, rest = tslib_es6.__rest(dto, ["channelAclDefinition"]);
...
const CHANNEL_ACTION_PRIVS = {
    // permissions
    UPDATE_OWNERS: [utils.Role.OWNER],
    UPDATE_MANAGERS: [utils.Role.OWNER, utils.Role.MANAGE],
    UPDATE_MODERATORS: [utils.Role.OWNER, utils.Role.MANAGE],
    UPDATE_ORGS: [utils.Role.OWNER, utils.Role.MANAGE],
    UPDATE_GROUPS: [utils.Role.OWNER, utils.Role.MANAGE],
    UPDATE_USERS: [utils.Role.OWNER, utils.Role.MANAGE],
    UPDATE_AUTHENTICATED_USERS: [utils.Role.OWNER, utils.Role.MANAGE],
    UPDATE_ANONYMOUS_USERS: [utils.Role.OWNER, utils.Role.MANAGE],
    // settings
    UPDATE_POST_REPLIES: [utils.Role.OWNER, utils.Role.MANAGE, utils.Role.MODERATE],
    UPDATE_POST_REACTIONS: [utils.Role.OWNER, utils.Role.MANAGE, utils.Role.MODERATE],
    UPDATE_POST_AS_ANONYMOUS: [utils.Role.OWNER, utils.Role.MANAGE, utils.Role.MODERATE],
    UPDATE_ALLOWED_REACTIONS: [utils.Role.OWNER, utils.Role.MANAGE, utils.Role.MODERATE],
    UPDATE_DEFAULT_POST_STATUS: [utils.Role.OWNER, utils.Role.MANAGE, utils.Role.MODERATE],
    UPDATE_BLOCKED_WORDS: [utils.Role.OWNER, utils.Role.MANAGE, utils.Role.MODERATE],
    UPDATE_CHANNEL_NAME: [utils.Role.OWNER, utils.Role.MANAGE],
    UPDATE_SOFT_DELETE_SETTING: [utils.Role.OWNER, utils.Role.MANAGE],
};
/**
 * @internal
 * @hidden
...
class ChannelPermission {
    ...
    canUpdateProperties(user, updateData = {}) {
        ...
        (this.isChanged(updates.allowReply, this.existingChannel.allowReply) &&
            ...
            // permissions
            (this.isRoleChanged(utils.Role.OWNER, updates.channelAcl) &&
                !CHANNEL_ACTION_PRIVS.UPDATE_OWNERS.includes(userRole)) ||
            (this.isRoleChanged(utils.Role.MANAGE, updates.channelAcl) &&
                !CHANNEL_ACTION_PRIVS.UPDATE_MANAGERS.includes(userRole)) ||
            (this.isRoleChanged(utils.Role.MODERATE, updates.channelAcl) &&
                !CHANNEL_ACTION_PRIVS.UPDATE_MODERATORS.includes(userRole)) ||
            (this.isCategoryChanged(utils.AclCategory.ORG, updates.channelAcl) &&
                !CHANNEL_ACTION_PRIVS.UPDATE_ORGS.includes(userRole)) ||
            (this.isCategoryChanged(utils.AclCategory.GROUP, updates.channelAcl) &&
                !CHANNEL_ACTION_PRIVS.UPDATE_GROUPS.includes(userRole)) ||
            (this.isCategoryChanged(utils.AclCategory.USER, updates.channelAcl) &&
                !CHANNEL_ACTION_PRIVS.UPDATE_USERS.includes(userRole)) ||
            (this.isCategoryChanged(utils.AclCategory.AUTHENTICATED_USER, updates.channelAcl) &&
                !CHANNEL_ACTION_PRIVS.UPDATE_AUTHENTICATED_USERS.includes(userRole)) ||
            (this.isCategoryChanged(utils.AclCategory.ANONYMOUS_USER, updates.channelAcl) &&
    ...
    canAnyUser(action) {
        var _c;
        const anonymousUserRole = (_c = this.permissionsByCategory[utils.AclCategory.ANONYMOUS_USER]) === null || _c === void 0 ? void 0 : _c[0].role;
        return channelActionLookup(action).includes(anonymousUserRole);
    }
    canAnyAuthenticatedUser(action) {
        var _c;
        const role = (_c = this.permissionsByCategory[utils.AclCategory.AUTHENTICATED_USER]) === null || _c === void 0 ? void 0 : _c[0].role;
        return channelActionLookup(action).includes(role);
    }
    canSomeUser(action, user) {
        var _c;
        const userPermissions = (_c = this.permissionsByCategory[utils.AclCategory.USER]) !== null && _c !== void 0 ? _c : [];
    ...
    canSomeUserGroup(action, user) {
        var _c;
        const groupAccessControls = (_c = this.permissionsByCategory[utils.AclCategory.GROUP]) !== null && _c !== void 0 ? _c : [];
    ...
    canSomeUserOrg(action, user) {
        var _c;
        const orgPermissions = (_c = this.permissionsByCategory[utils.AclCategory.ORG]) !== null && _c !== void 0 ? _c : [];
    ...
    userCanAddAnonymousToAcl(user) {
        if (!this.permissionsByCategory[utils.AclCategory.ANONYMOUS_USER]) {
            return true;
        }
    ...
    userCanAddUnauthenticatedToAcl(user) {
        if (!this.permissionsByCategory[utils.AclCategory.AUTHENTICATED_USER]) {
            return true;
        }
        return (isOrgAdmin(user) ||
            userHasPrivilege(user, "portal:admin:shareToPublic") ||
    ...
    userCanAddAllGroupsToAcl(user) {
        const groupPermissions = this.permissionsByCategory[utils.AclCategory.GROUP];
        const userGroupsById = this.mapUserGroupsById(user.groups);
        if (!groupPermissions) {
            return true;
        }
    ...
    userCanAddAllOrgsToAcl(user) {
        const orgPermissions = this.permissionsByCategory[utils.AclCategory.ORG];
        if (!orgPermissions) {
            return true;
        }
        return ((isOrgAdmin(user) ||
    ...
    userCanAddUsersToAcl(user) {
        const userPermissions = this.permissionsByCategory[utils.AclCategory.USER];
    ...
    determineUserRole(user) {
        if (this.isOwner(user)) {
            return utils.Role.OWNER;
        }
        else if (this.isManager(user)) {
            return utils.Role.MANAGE;
        }
        else if (this.isModerator(user)) {
            return utils.Role.MODERATE;
        }
        else {
            return utils.Role.READ;
        }
    }
    isOwner(user) {
...
function isGroupDiscussable(userGroup) {
    ...
    return !typeKeywords.includes(utils.CANNOT_DISCUSS);
}
function doesPermissionAllowGroupMemberType(permission, group) {
    if (permission.category !== utils.AclCategory.GROUP ||
    ...
    group.userMembership.memberType === "owner" ||
        ...
        permission.subCategory === utils.AclSubCategory.MEMBER);
}
function doesPermissionAllowOrgRole(permission, user) {
    return (permission.category === utils.AclCategory.ORG &&
        (permission.subCategory === utils.AclSubCategory.MEMBER ||
            (permission.subCategory === utils.AclSubCategory.ADMIN && isOrgAdmin(user))));
}
function channelActionLookup(action) {
    if (action === ChannelAction.WRITE_POSTS) {
        return [utils.Role.WRITE, utils.Role.READWRITE, utils.Role.MODERATE, utils.Role.MANAGE, utils.Role.OWNER];
    }
    if (action === ChannelAction.MODERATE_CHANNEL) {
        return [utils.Role.MODERATE, utils.Role.MANAGE, utils.Role.OWNER];
    }
    ...
    return [utils.Role.READ, utils.Role.READWRITE, utils.Role.MODERATE, utils.Role.MANAGE, utils.Role.OWNER];
}
