...
function generateToken(url, requestOptions) {
    ...
    if (typeof window !== "undefined" &&
        ...
        options.params.referer = request.NODEJS_DEFAULT_REFERER_HEADER;
    }
...
var UserSession = /** @class */ (function () {
    ...
    UserSession.beginOAuth2 = function (options, win) {
        ...
        if (params) {
            url = url + "&" + request.encodeQueryString(params);
        }
        if (!popup) {
        ...
        win["__ESRI_REST_AUTH_HANDLER_" + clientId] = function (errorString, oauthInfoString) {
            if (errorString) {
                ...
                session.reject(new request.ArcGISAuthError(error.errorMessage, error.error));
                return;
            }
            if (oauthInfoString) {
    ...
    UserSession.completeOAuth2 = function (options, win) {
        ...
        function completeSignIn(error, oauthInfo) {
            try {
                ...
                throw new request.ArcGISAuthError("Unable to complete authentication. It's possible you specified popup based oAuth2 but no handler from \"beginOAuth2()\" present. This generally happens because the \"popup\" option differs between \"beginOAuth2()\" and \"completeOAuth2()\".");
            }
            if (error) {
                throw new request.ArcGISAuthError(error.errorMessage, error.error);
            }
    ...
    UserSession.prototype.getUser = function (requestOptions) {
        ...
        if (this._pendingUserRequest) {
            ...
            this._pendingUserRequest = request.request(url, options).then(function (response) {
    ...
    UserSession.prototype.getPortal = function (requestOptions) {
        ...
        if (this._pendingPortalRequest) {
            ...
            var options = __assign(__assign({ httpMethod: "GET", authentication: this }, requestOptions), { rawResponse: false });
            this._pendingPortalRequest = request.request(url, options).then(function (response) {
    ...
    UserSession.prototype.refreshSession = function (requestOptions) {
        ...
        return Promise.reject(new request.ArcGISAuthError("Unable to refresh token."));
    };
    /**
    ...
    UserSession.prototype.getTokenForServer = function (url, requestOptions) {
        ...
        this._pendingTokenRequests[root] = this.fetchAuthorizedDomains().then(function () {
            return request.request(root + "/rest/info", {
                ...
                if (response.owningSystemUrl) {
                    ...
                    if (!isFederated(response.owningSystemUrl, _this.portal)) {
                        throw new request.ArcGISAuthError(url + " is not federated with " + _this.portal + ".", "NOT_FEDERATED");
                    }
                    else {
                        /**
                         * if the server is federated, use the relevant token endpoint.
                         */
                        return request.request(response.owningSystemUrl + "/sharing/rest/info", requestOptions);
                    }
                }
                    ...
                    throw new request.ArcGISAuthError(url + " is not federated with any portal and is not explicitly trusted.", "NOT_FEDERATED");
                }
            })
                .then(function (response) {
