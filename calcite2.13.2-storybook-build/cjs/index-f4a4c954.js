...
const defaultLocale = interfaces.SupportedLocales.English;
/*
...
function convertToSupportedLocale(locale) {
  let output = defaultLocale;
  if (locale !== undefined) {
    const supportedLocales = Object.values(interfaces.SupportedLocales);
...
function convertToSupportedFormatLocale(locale) {
  let formatLocale = interfaces.SupportedLocalesForFormats.find((item) => item === locale);
...
function getLocaleInfo(element = document.documentElement) {
  ...
  const direction = getDirectionFromElement(element);
  return getLocaleInfoFromLocale(locale, direction);
}
function getLocaleInfoFromLocale(locale, dir) {
  const convertedLocale = convertToSupportedLocale(locale);
  const formatLocale = convertToSupportedFormatLocale(locale);
  let rtl = interfaces.RTLLocales.includes(convertedLocale);
  let direction = dir !== undefined ? dir : (rtl === true ? "rtl" : "ltr");
  ...
  return { locale: convertedLocale, rtl, formatLocale, direction };
}
function getDirectionFromElement(element) {
  let direction = element.getAttribute("dir");
  if (direction !== "ltr" && direction !== "rtl") {
    const { rtl } = getLocaleInfoFromLocale(window.navigator.language);
    direction = rtl === true ? "rtl" : "ltr";
  }
  return direction;
}
...
async function fetchLocaleStrings(props) {
  ...
  const defaultResponse = await fetch(`${baseURL}/${getFileName(defaultLocale)}`);
  ...
  if (locale !== defaultLocale) {
    try {
      ...
      console.warn(`Unable to fetch locale file for ${path} component for locale ${locale}`);
    }
  }
  return mergeDeep(defaultData, localeData);
}
/**
...
exports.fetchComponentLocaleStrings = fetchComponentLocaleStrings;
exports.fetchLanguageDirection = fetchLanguageDirection;
exports.getLocaleInfo = getLocaleInfo;
exports.getLocaleInfoFromLocale = getLocaleInfoFromLocale;
exports.localeParseFloat = localeParseFloat;
