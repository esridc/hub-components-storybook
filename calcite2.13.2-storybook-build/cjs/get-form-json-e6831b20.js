...
function deepEqual(a, b) {
    ...
    // object checks
    if (a && b && _deepMapValues._isObject(a) && _deepMapValues._isObject(b)) {
...
function getPropertyMap$3() {
    const map = HubInitiatives.getBasePropertyMap();
...
function computeProps$3(model, discussion, requestOptions) {
    ...
    discussion = HubInitiatives.computeItemProps(model.item, discussion);
    // thumbnail url
    discussion.thumbnailUrl = compose.getItemThumbnailUrl(model.item, requestOptions, token);
...
const DEFAULT_ENTITY_SETTINGS_BY_ENTITY_TYPE = {
    discussion: {
        type: utils.EntitySettingType.CONTENT,
...
function computeProps$2(model, content, requestOptions, enrichments = {}) {
    ...
    content = HubInitiatives.computeItemProps(model.item, content);
    // thumbnail url
    ...
    content.links = {
        self: getItemHomeUrl.getItemHomeUrl(content.id, requestOptions),
        siteRelative: compose.getHubRelativeUrl(content.type, content.slug || content.id, content.typeKeywords),
        siteRelativeEntityType: compose.getHubRelativeUrl(content.type),
        workspaceRelative: getRelativeWorkspaceUrl.getRelativeWorkspaceUrl("content", content.id),
        thumbnail: thumbnailUrl,
        contentEditUrl: compose.getContentEditUrl(model.item, requestOptions),
    };
    ...
    content.extendedProps = isService.isService(content.url)
    ...
    if (enrichments.server) {
        content.serverQueryCapability = hostedServiceUtils.hasServiceCapability(hostedServiceUtils.ServiceCapabilities.QUERY, enrichments.server);
        content.serverExtractCapability = hostedServiceUtils.hasServiceCapability(hostedServiceUtils.ServiceCapabilities.EXTRACT, enrichments.server);
        const extractFormatsList = getProp.getProp(enrichments, "server.supportedExportFormats");
    ...
    if (enrichments.metadata) {
        content.additionalResources = compose.getAdditionalResources(model.item, enrichments.metadata, requestOptions);
    }
    return content;
}
/**
...
function getServiceExtendedProps(item, enrichments, requestOptions) {
    ...
    if (enrichments.server) {
        ...
        result.serverQueryCapability = hostedServiceUtils.hasServiceCapability(hostedServiceUtils.ServiceCapabilities.QUERY, enrichments.server);
        result.serverExtractCapability = hostedServiceUtils.hasServiceCapability(hostedServiceUtils.ServiceCapabilities.EXTRACT, enrichments.server);
        const extractFormatsList = getProp.getProp(enrichments, "server.supportedExportFormats");
...
function getBaseExtendedProps(item, enrichments, requestOptions) {
    return {
        ...
        additionalResources: compose.getAdditionalResources(item, enrichments.metadata, requestOptions),
        downloads: getProp.getProp(item, "properties.downloads"),
    };
}
...
const getSchedule = async (itemId, requestOptions) => {
    const fetchResponse = await fetch(compose.getSchedulerApiUrl(itemId, requestOptions));
...
const setSchedule = async (itemId, schedule, requestOptions) => {
    const body = util.cloneObject(schedule);
    if (body.mode !== "manual") {
        // remove mode if not manual
        delete body.mode;
    }
    const url = compose.getSchedulerApiUrl(itemId, requestOptions);
    const options = {
        method: "POST",
        headers: {
...
function computeLinks(item, requestOptions) {
    return HubInitiatives.computeItemLinks(item, requestOptions);
}
...
function computeProps$1(model, initiativeTemplate, requestOptions) {
    ...
    initiativeTemplate = HubInitiatives.computeItemProps(model.item, initiativeTemplate);
    // thumbnail url
    initiativeTemplate.thumbnailUrl = compose.getItemThumbnailUrl(model.item, requestOptions, token);
    /**
     ...
     */
    initiativeTemplate.features = HubInitiatives.processEntityFeatures(((_a = model.data.settings) === null || _a === void 0 ? void 0 : _a.features) || {}, InitiativeTemplateBusinessRules.InitiativeTemplateDefaultFeatures);
...
function computeProps(model, survey, requestOptions) {
    ...
    survey = HubInitiatives.computeItemProps(model.item, survey);
    // thumbnail url
    survey.thumbnailUrl = compose.getItemThumbnailUrl(model.item, requestOptions, token);
    // map props
...
const getFormInfoJson = (id, requestOptions) => {
    return get.getItemInfo(id, Object.assign({ fileName: "forminfo.json", readAs: "json" }, requestOptions));
};
...
const decodeForm = (form) => {
    const target = util.cloneObject(form);
    const props = [
        "header.content",
        "subHeader.content",
        "footer.content",
    ...
    props.forEach((prop) => {
        if (getProp.getProp(target, prop)) {
            setProp.setProp(prop, decodeURIComponent(getProp.getProp(target, prop)), target);
        }
    });
...
const getFormJson = async (item, requestOptions) => {
    ...
    if (isSurvey123Connect(item)) {
        ...
        promise = Promise.all([
            get.getItemInfo(item.id, Object.assign({ fileName: `${name}.webform`, readAs: "json" }, requestOptions)),
            get.getItemInfo(item.id, Object.assign({ fileName: `${name}.info`, readAs: "json" }, requestOptions)),
        ]).then(([webform, info]) => {
        ...
        promise = get.getItemInfo(item.id, Object.assign({ fileName: `${name}.json`, readAs: "json" }, requestOptions));
    }
...
exports.MAP_SURVEY_TYPEKEYWORD = MAP_SURVEY_TYPEKEYWORD;
exports.computeLinks = computeLinks;
exports.computeProps = computeProps$3;
exports.computeProps$1 = computeProps$1;
exports.computeProps$2 = computeProps;
exports.decodeForm = decodeForm;
exports.deepEqual = deepEqual;
exports.getDefaultEntitySettings = getDefaultEntitySettings;
exports.getFormInfoJson = getFormInfoJson;
exports.getFormJson = getFormJson;
exports.getMapQuestion = getMapQuestion;
...
exports.hasMapQuestion = hasMapQuestion;
exports.isDownloadSchedulingAvailable = isDownloadSchedulingAvailable;
exports.isDraft = isDraft;
exports.isMapQuestion = isMapQuestion;
exports.isPageQuestion = isPageQuestion;
exports.isSurvey123Connect = isSurvey123Connect;
exports.maybeUpdateSchedule = maybeUpdateSchedule;
exports.modelToHubEditableContent = modelToHubEditableContent;
exports.shouldDisplayMap = shouldDisplayMap;
