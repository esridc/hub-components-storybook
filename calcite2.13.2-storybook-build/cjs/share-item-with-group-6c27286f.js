...
function isItemSharedWithGroup(requestOptions) {
    ...
    return search.searchItems(searchOpts).then(function (searchResponse) {
        var result = false;
    ...
    return isItemSharedWithGroup(requestOptions)
        ...
        if (itemOwner !== username) {
            ...
            return Promise.all([
                ...
                helpers.getUserMembership(requestOptions),
            ])
                .then(function (_a) {
...
function shareToGroup(requestOptions, isAdmin, isCrossOrgSharing) {
    ...
    var url = getPortalUrl.getPortalUrl(requestOptions) + "/content/items/" + requestOptions.id + "/share";
    ...
    if (itemOwner === username || (isAdmin && !isCrossOrgSharing)) {
        url = getPortalUrl.getPortalUrl(requestOptions) + "/content/users/" + itemOwner + "/items/" + requestOptions.id + "/share";
    }
...
function ensureMembership(currentUser, ownerUser, shouldPromote, errorMessage, requestOptions) {
    ...
    if (group) {
        ...
        if (shouldPromote && group.userMembership.memberType === "member") {
            // promote them
            promise = updateUserMembership.updateUserMemberships({
            ...
            revert = function (sharingResults) {
                return updateUserMembership.updateUserMemberships({
                    id: requestOptions.groupId,
                    users: [ownerUser.username],
        ...
        // the api error
        promise = updateUserMembership.addGroupUsers((_a = {
                id: requestOptions.groupId
            },
            ...
            .catch(function () { return ({ notAdded: [ownerUser.username] }); });
        revert = function (sharingResults) {
            return updateUserMembership.removeGroupUsers({
                id: requestOptions.groupId,
                users: [ownerUser.username],
                authentication: requestOptions.authentication,
            }).then(function () {
