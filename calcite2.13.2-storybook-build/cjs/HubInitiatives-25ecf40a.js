...
const slugs$1 = require('./slugs-8f743e2c.js');
...
includeIdInSlug = false) {
    ...
    return slug
        ? slugs.keywordSlugToUriSlug(includeIdInSlug ? slugs.appendIdToSlug(slug, id) : slug)
        : id;
}
...
function editorToEntity(editor, portal) {
    ...
    if (_slug) {
        ...
        entity.slug = slugs.truncateSlug(_slug, entity.orgUrlKey);
    }
    return entity;
}
...
const ensureUniqueEntitySlug = async (entity, requestOptions) => {
    ...
    entity.slug = await themes.getUniqueSlug(slugInfo, requestOptions);
    // add slug to keywords
    entity.typeKeywords = themes.setSlugKeyword(entity.typeKeywords, entity.slug);
    return entity;
};
...
function computeItemProps(item, entity) {
    ...
    // TODO: thumbnail url?
    // location
    entity.location = entity.location || compose.deriveLocationFromItem(item);
    // isDiscussable
    entity.isDiscussable = utils.isDiscussable(item);
...
async function enrichContentSearchResult(item, include, requestOptions) {
    ...
    // }
    // merge includes
    include = [...DEFAULTS, ...include].filter(util.unique);
    ...
    if (enrichments.length) {
        enriched = await _enrichments.fetchItemEnrichments(item, enrichments, requestOptions);
    }
    // map the enriched props onto the result
    specs.forEach((spec) => {
        result[spec.prop] = getProp.getProp(enriched, spec.path);
    });
    // Handle links
    // TODO: Link handling should be an enrichment
    result.links.thumbnail = compose.getItemThumbnailUrl(item, requestOptions);
    result.links.self = getItemHomeUrl.getItemHomeUrl(result.id, requestOptions);
    result.links.siteRelative = compose.getHubRelativeUrl(result.type, result.id, item.typeKeywords);
    result.links.workspaceRelative = getRelativeWorkspaceUrl.getRelativeWorkspaceUrl(result.type, result.id);
    return result;
}
...
function fetchItem(identifier, requestOptions) {
    const { id, slug, orgKey } = slugs.parseIdentifier(identifier);
    if (id) {
        // use the id to fetch the item
        return get.getItem(id, requestOptions);
    }
    // we'll have to look up by slug
    // first, ensure the slug has the org key
    const fullyQualifiedSlug = slugs$1.addContextToSlug(slug, orgKey || requestOptions.siteOrgKey);
    const slugKeyword = slugs.uriSlugToKeywordSlug(fullyQualifiedSlug);
    return themes.findItemsBySlug({ slug: slugKeyword }, requestOptions).then((results) => {
        if (results.length) {
...
function computeItemLinks(item, requestOptions) {
    ...
    return {
        self: getItemHomeUrl.getItemHomeUrl(item.id, requestOptions),
        siteRelative: compose.getHubRelativeUrl(item.type, 
        // use slug if available, otherwise id
        getItemIdentifier(item), item.typeKeywords),
        siteRelativeEntityType: compose.getHubRelativeUrl(item.type),
        // no SEO for workspace, so we always use id instead of slug
        workspaceRelative: getRelativeWorkspaceUrl.getRelativeWorkspaceUrl(item.type, item.id),
        thumbnail: compose.getItemThumbnailUrl(item, requestOptions, token),
    };
}
...
function upgradeCatalogSchema(catalog) {
    if (getProp.getProp(catalog, "schemaVersion") === CATALOG_SCHEMA_VERSION) {
        return catalog;
    }
    else {
        let clone = util.cloneObject(catalog);
...
function applyCatalogSchema(original) {
    if (getProp.getProp(original, "schemaVersion") > 1.0) {
        return original;
    }
    else {
        const catalog = {
        ...
        const rawGroups = getProp.getProp(original, "groups") || [];
        let groups = [];
        ...
        const orgId = getProp.getProp(original, "orgId");
        if (orgId) {
...
function computeProps$4(model, initiative, requestOptions) {
    ...
    // thumbnail url
    initiative.thumbnailUrl = compose.getItemThumbnailUrl(model.item, requestOptions, token);
     ...
     */
    initiative.features = processEntityFeatures(((_b = model.data.settings) === null || _b === void 0 ? void 0 : _b.features) || {}, TemplateBusinessRules.InitiativeDefaultFeatures);
...
function applyInitiativeMigrations(model, requestOptions) {
    if (getProp.getProp(model, "item.properties.schemaVersion") ===
...
function addDefaultCatalog(model) {
    if (getProp.getProp(model, "item.properties.schemaVersion") >= 1.1) {
        ...
        const groupId = getProp.getProp(clone, "item.properties.contentGroupId");
...
function getNextFunction(request, nextStart, total, fn) {
    var _a;
    const clonedRequest = util.cloneObject(request);
    // clone will not handle authentication so we do it manually
    if (request.authentication) {
        clonedRequest.authentication = UserSession.UserSession.deserialize(request.authentication.serialize());
    }
...
function getResultSiteRelativeLink(searchResult, site) {
    ...
    if (siteRelativeLink && compose.isPageType(type, typeKeywords)) {
...
function addDefaultItemSearchPredicates(query) {
    const queryWithDefaultItemPredicates = util.cloneObject(query);
...
function expandPredicate(predicate) {
    ...
    Object.entries(predicate).forEach(([key, value]) => {
        ...
        if (!PREDICATE_NON_MATCH_OPTIONS_PROPS.includes(key)) {
            setProp.setProp(key, valueToMatchOptions(value), result);
        }
        // Handle Date fields
        if (PREDICATE_DATE_PROPS.includes(key)) {
            const dateFieldValue = util.cloneObject(getProp.getProp(predicate, key));
            if (getProp.getProp(predicate, `${key}.type`) === "relative-date") {
                setProp.setProp(key, relativeDateToDateRange(dateFieldValue), result);
            }
            else {
                setProp.setProp(key, dateFieldValue, result);
            }
        }
        // Handle fields that are just copied forward
        if (PREDICATE_COPY_PROPS.includes(key) && predicate.hasOwnProperty(key)) {
            setProp.setProp(key, value, result);
        }
    });
    return result;
}
...
function computeLinks$3(item, requestOptions) {
    ...
    const siteRelative = compose.getHubRelativeUrl(item.type, getItemIdentifier(item, true), item.typeKeywords);
...
function computeProps$3(model, page, requestOptions) {
    ...
    // thumbnail url
    const thumbnailUrl = compose.getItemThumbnailUrl(model.item, requestOptions, token);
     ...
     */
    page.features = processEntityFeatures(((_a = model.data.settings) === null || _a === void 0 ? void 0 : _a.features) || {}, TemplateBusinessRules.PageDefaultFeatures);
...
async function createPage(partialPage, requestOptions) {
    ...
    if (!page.slug) {
        page.slug = themes.constructSlug(page.name, page.orgUrlKey);
    }
    // Ensure slug is  unique
    await ensureUniqueEntitySlug(page, requestOptions);
    // Map page object onto a default page Model
    const mapper = new PropertyMapper.PropertyMapper(getPropertyMap$4());
    // create model from object, using the default model as a starting point
    let model = mapper.entityToStore(page, util.cloneObject(DEFAULT_PAGE_MODEL));
    // create the item
...
async function updatePage(page, requestOptions) {
    ...
    const model = await themes.getModel(page.id, requestOptions);
...
function convertHubGroupToGroup(hubGroup) {
    ...
    const mapper = new PropertyMapper.PropertyMapper(getPropertyMap$3());
...
async function removeDomainByHostname(hostname, hubRequestOptions) {
    ...
    try {
        const domainEntry = await themes.lookupDomain(hostname, hubRequestOptions);
        // Could consider doing a check here to verify that current user
        // is member of the org owning the domain record, but api will
        // enforce this.
        const id = getProp.getProp(domainEntry, "id");
        if (id) {
            await themes.removeDomain(id, hubRequestOptions);
        }
    }
    catch (ex) {
...
function fetchSiteModel(identifier, hubRequestOptions) {
    ...
    if (isGuid.isGuid(identifier)) {
        prms = themes.getSiteById(identifier, hubRequestOptions);
    }
    else {
        let hostnameOrSlug = identifier;
        // get down the the hostname
        hostnameOrSlug = domainExists.stripProtocol(hostnameOrSlug);
        hostnameOrSlug = hostnameOrSlug.split("/")[0];
        prms = themes.lookupDomain(hostnameOrSlug, hubRequestOptions).then(({ siteId }) => themes.getSiteById(siteId, hubRequestOptions));
    }
    return prms;
}
...
async function handleDomainChanges(updatedModel, currentModel, requestOptions) {
    ...
    ["customHostname", "defaultHostname"].forEach((key) => {
        const currentValue = getProp.getProp(currentModel, `data.values.${key}`) || "";
        const updatedValue = getProp.getProp(updatedModel, `data.values.${key}`) || "";
    ...
    domainChanges.add.map((hostname) => {
        ...
        domainChangePromises.push(themes.addDomain(domainOpts, requestOptions));
    });
    // handle removals
...
function applyPermissionMigration(model) {
    ...
    permissionMigrations.forEach((defn) => {
        const value = getProp.getProp(clone, defn.prop);
        if (value) {
...
function computeProps$2(model, site, requestOptions) {
     ...
     * Features that can be disabled by the entity owner
     */
    site.features = processEntityFeatures(((_a = model.data.settings) === null || _a === void 0 ? void 0 : _a.features) || {}, TemplateBusinessRules.SiteDefaultFeatures);
...
function applyCatalogStructureMigration(model) {
    const siteCatalog = getWithDefault.getWithDefault(model.data, "catalog", {});
    ...
    if (!siteCatalog.schemaVersion) {
        const clonedModel = util.cloneObject(model);
...
function convertCatalogToLegacyFormat(modelToUpdate, currentModel) {
    const updatedModel = util.cloneObject(modelToUpdate);
    ...
    if (legacyCatalog.groups.length) {
        ...
        updatedModel.data.catalog = util.cloneObject(currentModel.data.catalog);
        return updatedModel;
    }
    return updatedModel;
}
...
function catalogToLegacy(catalog) {
    ...
    if ((_a = catalog.scopes) === null || _a === void 0 ? void 0 : _a.item) {
        ...
        if (groupPredicate) {
            ...
            legacyCatalog.groups = getWithDefault.getWithDefault(groupPredicate, "group.any", []);
        }
    }
...
const convertFeaturesToLegacyCapabilities = (modelToUpdate, currentModel) => {
    ...
    (getProp.getProp(currentModel, "data.values.capabilities") || []).forEach((capability) => {
        legacyCapabilityFeatureFlags[capability] = true;
    });
    // 2. override legacy capabilities that are driven by features
    const features = getProp.getProp(modelToUpdate, "data.settings.features") || {};
    legacyCapabilityFeatureFlags = themes.capabilityToFeatureMap.reduce((capabilities, map) => {
    ...
    setProp.setProp("data.values.capabilities", updatedCapabilities, modelToUpdate);
...
async function createSite(partialSite, requestOptions) {
    ...
    // Create a slug from the title if one is not passed in
    if (!site.slug) {
        site.slug = themes.constructSlug(site.name, site.orgUrlKey);
    }
    // Ensure slug is  unique
    await ensureUniqueEntitySlug(site, requestOptions);
    if (!site.subdomain) {
        site.subdomain = slugify.slugify(site.name);
    }
    site.subdomain = await themes.ensureUniqueDomainName(site.subdomain, requestOptions);
    // Domains
    if (!requestOptions.isPortal) {
        // now that we know the subdomain is available, set the defaultHostname
        site.defaultHostname = `${site.subdomain}-${portal.urlKey}.${domainExists.stripProtocol(compose.getHubApiUrl(requestOptions))}`;
        // set the url
    ...
    if (!site.theme) {
        site.theme = themes.getOrgDefaultTheme(portal);
    }
    ...
    if (!getProp.getProp(site, "map.basemaps.primary")) {
        setProp.setProp("map.basemaps.primary", portal.defaultBasemap, site);
    }
    // Put the title into the header
    if (!getProp.getProp(site, "layout.header.component.settings.title")) {
        setProp.setProp("layout.header.component.settings.title", site.name, site);
    }
    site.typeKeywords = utils.setDiscussableKeyword(site.typeKeywords, site.isDiscussable);
    // Now convert the IHubSite into an IModel
    const mapper = new PropertyMapper.PropertyMapper(getPropertyMap$2());
    let model = mapper.entityToStore(site, util.cloneObject(DEFAULT_SITE_MODEL));
    ...
    if (getProp.getProp(model, "data.catalog.scopes")) {
    ...
    const domainResponses = await themes.addSiteDomains(model, requestOptions);
    model.data.values.clientId = domainResponses[0].clientKey;
    // update the model
    const updatedModel = await themes.updateModel(model, requestOptions);
...
async function updateSite(site, requestOptions) {
    ...
    const currentModel = await themes.getModel(site.id, requestOptions);
    // Note: Although we are fetching the model, and applying changes onto it,
    // we are not attempting to handle "concurrent edit" conflict resolution
    // but this is where we would apply that sort of logic
    const mapper = new PropertyMapper.PropertyMapper(getPropertyMap$2());
    ...
    const updatedSiteModel = await themes.updateModel(modelToUpdate, requestOptions);
...
async function deleteSite(id, requestOptions) {
    ...
    if (!requestOptions.isPortal) {
        await themes.removeDomainsBySiteId(id, requestOptions);
    }
...
function computeProps$1(model, project, requestOptions) {
    ...
    // thumbnail url
    project.thumbnailUrl = compose.getItemThumbnailUrl(model.item, requestOptions, token);
     ...
     * Features that can be disabled by the entity owner
     */
    project.features = processEntityFeatures(((_b = model.data.settings) === null || _b === void 0 ? void 0 : _b.features) || {}, TemplateBusinessRules.ProjectDefaultFeatures);
...
async function convertItemToProject(item, requestOptions) {
    const model = await themes.fetchModelFromItem(item, requestOptions);
    // TODO: In the future we will handle the boundary fetching from resource
    const mapper = new PropertyMapper.PropertyMapper(getPropertyMap$1());
    const prj = mapper.storeToEntity(model, {});
...
function editorToMetric(values = {
    ...
    const { value, valueType, dynamicMetric } = values, config = tslib_es6.__rest(values, ["value", "valueType", "dynamicMetric"]);
    ...
    // create card config
    const displayConfig = Object.assign(Object.assign({}, config), { displayType: config.displayType || "stat-card", visibility: config.visibility || Metrics.MetricVisibility.hidden, metricId,
    ...
    const whereClause = expressionSet
        ...
        switch (field.type) {
            case "esriFieldTypeString":
                ...
                clause =
                    relationship === Metrics.ExpressionRelationships.LIKE
            ...
            default:
                // just the first bounding value
                if (!util.isNil(values[0])) {
                    clause = `(${field.name}) >= ${values[0]}`;
                }
                // just the second bounding value
                if (!util.isNil(values[1])) {
                    clause = `(${field.name}) <= ${values[1]}`;
                }
                // if we have both, rewrite clause
                if (!util.isNil(values[0]) && !util.isNil(values[1])) {
...
function setMetricAndDisplay(entity, metric, displayConfig) {
    const entityCopy = util.cloneObject(entity);
    const metricId = metric.id;
    // get with default in case we have an undefined metrics array
    const metrics = getWithDefault.getWithDefault(entityCopy, "metrics", []);
    const mIndex = metrics.findIndex((m) => m.id === metricId);
    // get array in case of undefined displays array
    const displays = getWithDefault.getWithDefault(entityCopy, "view.metricDisplays", []);
...
const getDeployedTemplateType = (template) => {
    ...
    if (hubSolutionTypeKeyword) {
        ...
        deployedType = dasherize.dasherize(camelizedType)
            .split("-")
            .map((str) => util.capitalize(str))
            .join(" ");
...
function computeProps(model, template, requestOptions) {
    ...
    template.features = processEntityFeatures(getProp.getProp(model, "data.settings.features") || {}, TemplateBusinessRules.TemplateDefaultFeatures);
    // 7. compute additional template-specific properties
    template.isDeployed = (getProp.getProp(model, "item.typeKeywords") || []).includes("Deployed");
...
async function convertItemToTemplate(item, requestOptions) {
    ...
    const mapper = new PropertyMapper.PropertyMapper(getPropertyMap());
...
function processSearchParams(options, query) {
    ...
    if (!options.requestOptions) {
        throw new HubError.HubError("portalSearchItems", "options.requestOptions is required.");
    }
...
async function searchPortalAsItem(searchOptions) {
    ...
    const resp = await search.searchItems(searchOptions);
...
function applyWellKnownCollectionFilters(query) {
    const updated = util.cloneObject(query);
    if (updated.collection) {
        const { collection, targetEntity, filters: queryFilters } = query;
        const wellKnownCollection = wellKnownCatalog.getWellknownCollection("", targetEntity, collection);
        const wellKnownFilters = getProp.getProp(wellKnownCollection, "scope.filters") || [];
...
function applyWellKnownItemPredicates(query) {
    const queryClone = util.cloneObject(query);
    // iterate the filters
                    ...
                    if (Array.isArray(types)) {
                        ...
                        predicate.type[key] = types.reduce((typesAcc, type) => {
                            if (isFamilyExpansionType(type)) {
                                ...
                                const familyTypes = getFamily.getFamilyTypes(family);
...
function isFamilyExpansionType(key) {
    ...
    if (key &&
        key.charAt(0) === "$" &&
        types.HubFamilies.includes(key.slice(1))) {
        result = true;
    }
    return result;
}
...
async function createInitiative(partialInitiative, requestOptions) {
    ...
    // Create a slug from the title if one is not passed in
    if (!initiative.slug) {
        initiative.slug = themes.constructSlug(initiative.name, initiative.orgUrlKey);
    }
    // Ensure slug is  unique
    ...
    initiative.typeKeywords = utils.setDiscussableKeyword(initiative.typeKeywords, initiative.isDiscussable);
    // Map initiative object onto a default initiative Model
    const mapper = new PropertyMapper.PropertyMapper(getPropertyMap$5());
    // create model from object, using the default model as a starting point
    let model = mapper.entityToStore(initiative, util.cloneObject(DEFAULT_INITIATIVE_MODEL));
    // create the item
    model = await themes.createModel(model, requestOptions);
...
async function updateInitiative(initiative, requestOptions) {
    ...
    initiative.typeKeywords = utils.setDiscussableKeyword(initiative.typeKeywords, initiative.isDiscussable);
    // get the backing item & data
    const model = await themes.getModel(initiative.id, requestOptions);
    // create the PropertyMapper
    const mapper = new PropertyMapper.PropertyMapper(getPropertyMap$5());
    // Note: Although we are fetching the model, and applying changes onto it,
    // we are not attempting to handle "concurrent edit" conflict resolution
...
async function convertItemToInitiative(item, requestOptions) {
    let model = await themes.fetchModelFromItem(item, requestOptions);
    // apply migrations
...
function getPendingProjectsQuery(initiative) {
    ...
    let query = getTypeWithKeywordQuery.getTypeWithKeywordQuery("Hub Project", `initiative|${initiative.id}`);
    // The the item scope from the catalog...
    const qry = getProp.getProp(initiative, "catalog.scopes.item");
    // negate the scope, combine that with the base query
    query = getTypeWithKeywordQuery.combineQueries([query, negateGroupPredicates(qry)]);
    return query;
}

exports.DEFAULT_INITIATIVE = DEFAULT_INITIATIVE;
exports.DEFAULT_PAGE = DEFAULT_PAGE;
exports.ENTERPRISE_PAGE_ITEM_TYPE = ENTERPRISE_PAGE_ITEM_TYPE;
exports.ENTERPRISE_SITE_ITEM_TYPE = ENTERPRISE_SITE_ITEM_TYPE;
exports.EntityResourceMap = EntityResourceMap;
exports.HUB_PAGE_ITEM_TYPE = HUB_PAGE_ITEM_TYPE;
exports.HUB_SITE_ITEM_TYPE = HUB_SITE_ITEM_TYPE;
exports.PREDICATE_DATE_PROPS = PREDICATE_DATE_PROPS;
exports.PREDICATE_NON_MATCH_OPTIONS_PROPS = PREDICATE_NON_MATCH_OPTIONS_PROPS;
exports.SEARCH_APIS = SEARCH_APIS;
exports.addDefaultItemSearchPredicates = addDefaultItemSearchPredicates;
exports.buildWhereClause = buildWhereClause;
exports.computeItemLinks = computeItemLinks;
exports.computeItemProps = computeItemProps;
exports.computeProps = computeProps$1;
exports.computeProps$1 = computeProps;
exports.convertHubGroupToGroup = convertHubGroupToGroup;
exports.convertItemToInitiative = convertItemToInitiative;
exports.convertItemToPage = convertItemToPage;
exports.convertItemToProject = convertItemToProject;
exports.convertItemToSite = convertItemToSite;
exports.convertItemToTemplate = convertItemToTemplate;
exports.convertModelToPage = convertModelToPage;
...
exports.editorToInitiative = editorToInitiative;
exports.editorToMetric = editorToMetric;
...
exports.ensureUniqueEntitySlug = ensureUniqueEntitySlug;
exports.expandApi = expandApi;
exports.expandApis = expandApis;
exports.expandPortalQuery = expandPortalQuery;
...
exports.getGroupPredicate = getGroupPredicate;
exports.getGroupThumbnailUrl = getGroupThumbnailUrl;
exports.getItemIdentifier = getItemIdentifier;
exports.getKilobyteSizeOfQuery = getKilobyteSizeOfQuery;
exports.getNextFunction = getNextFunction;
exports.getPendingProjectsQuery = getPendingProjectsQuery;
exports.getPropertyMap = getPropertyMap$3;
exports.getPropertyMap$1 = getPropertyMap$1;
exports.getPropertyMap$2 = getPropertyMap;
exports.getPropertyMap$3 = getPropertyMap$2;
exports.getResultSiteRelativeLink = getResultSiteRelativeLink;
exports.getScopeGroupPredicate = getScopeGroupPredicate;
...
exports.itemToSearchResult = itemToSearchResult;
exports.migrateToCollectionKey = migrateToCollectionKey;
exports.negateGroupPredicates = negateGroupPredicates;
exports.parseInclude = parseInclude;
exports.portalSearchItems = portalSearchItems;
exports.portalSearchItemsAsItems = portalSearchItemsAsItems;
exports.processEntityFeatures = processEntityFeatures;
...
exports.setEntityStatusKeyword = setEntityStatusKeyword;
