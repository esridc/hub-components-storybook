...
const index = require('./index-f4a4c954.js');
...
class _IntlManager {
  ...
  getIntlForComponent(element) {
    ...
    const { direction, formatLocale } = index.getLocaleInfo(element);
  ...
  async loadIntlForComponent(element, baseUrl) {
    // determine component locale
    const { locale, direction, formatLocale } = index.getLocaleInfo(element);
    // fetch messages for this component
    // TODO: lazy load these
    const { fileNamePattern, assetPath } = this;
    const { strings } = await index.fetchComponentLocaleStrings(element, baseUrl || index$1.getAssetPath(assetPath), fileNamePattern);
