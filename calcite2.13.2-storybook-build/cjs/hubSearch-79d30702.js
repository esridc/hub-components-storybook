...
const search$1 = require('./search-b00c4c79.js');
...
const request$1 = require('./request-79b61e92.js');
 ...
 * @returns A Promise that will resolve with the data from the response.
 */
function searchCommunityUsers$1(search$1) {
    return search.genericSearch(search$1, "communityUser");
}
...
function pickProps(obj, props) {
    return mergeObjects.mergeObjects(obj, {}, props);
}
...
function getApi(targetEntity, options) {
    ...
    let result;
    if (api) {
        result = HubInitiatives.expandApi(api);
    }
...
const enrichUserOrg = (input) => {
    ...
    if (!data.user.orgId) {
        ...
        const options = Object.assign(Object.assign({}, requestOptions), { 
            ...
            portal: `${getPortalBaseFromOrgUrl.getPortalBaseFromOrgUrl(requestOptions.portal)}/sharing/rest` });
        // TODO: Add Caching
        return fetchOrg.fetchOrg(data.user.orgId, options)
            .then((results) => {
            stack.finish(opId);
            return {
                data: Object.assign(Object.assign({}, data), {
...
const handleEnrichmentError$1 = (error, input, opId) => {
    ...
    return {
        data: Object.assign(Object.assign({}, data), { errors: _enrichments.getEnrichmentErrors(error, data.errors) }),
        stack,
        requestOptions,
    };
};
...
function fetchUserEnrichments(user, enrichments, requestOptions) {
    ...
    const pipeline = _enrichments.createOperationPipeline(operations);
    // execute pipeline and return the item and enrichments
    return pipeline({
        data: { user },
        stack: new _enrichments.OperationStack(),
        requestOptions,
    }).then((output) => {
...
async function computeProps$1(model, user, context) {
    ...
    const fsGetSignInSettings = failSafe.failSafe(getPortalSignInSettings, {});
    const signinSettings = await fsGetSignInSettings(context);
    const fsGetSelf = failSafe.failSafe(getPortal.getSelf, {});
...
function getPortalSignInSettings(context) {
    const url = `${getPortalUrl$1.getPortalUrl(context.requestOptions)}/portals/self/signinSettings`;
    const requestOpts = Object.assign(Object.assign({}, context.requestOptions), { httpMethod: "GET" });
    return request.request(url, requestOpts);
}
...
async function enrichUserSearchResult(user, include, requestOptions) {
    ...
    const specs = include.filter(util.unique).map(HubInitiatives.parseInclude);
    ...
    if (user.thumbnail) {
        result.links.thumbnail = HubInitiatives.getUserThumbnailUrl(requestOptions.portal, user, token);
    }
...
const fetchHubUser = async (username, context) => {
    ...
    user =
        username === "self"
            ? context.currentUser
            : await getUser.getUser(Object.assign(Object.assign({}, context.hubRequestOptions), { username }));
...
async function portalSearchGroupMembers(query, options) {
    ...
    query.filters.forEach((filter) => {
        filter.predicates.forEach((predicate) => {
            const prop = getProp.getProp(predicate, "group");
            if (Array.isArray(prop)) {
                ...
                groupId = getProp.getProp(prop, "any[0]") || getProp.getProp(prop, "all[0]");
            }
        });
    });
    if (!groupId) {
        throw new HubError.HubError("portalSearchGroupMembers", "Group Id required. Please pass as a predicate in the query.");
    }
    ...
    query.filters = query.filters.map((filter) => {
        ...
        filter.predicates = filter.predicates
            ...
            .map(HubInitiatives.expandPredicate);
        return filter;
    });
    // Serialize the all the groups for portal
    const so = HubInitiatives.serializeQueryForPortal(query);
...
async function searchGroupMembers(searchOptions) {
    ...
    const resp = await get.searchGroupUsers(searchOptions.groupId, opts);
    ...
    return {
        ...
        next: HubInitiatives.getNextFunction(searchOptions, resp.nextStart, resp.total, searchGroupMembers),
    };
}
/**
...
async function memberToSearchResult(member, include = [], requestOptions) {
    ...
    const fsGetUser = failSafe.failSafe(getUser.getUser, user);
    ...
    Object.keys(user).forEach((key) => {
        if (fetchedUser.hasOwnProperty(key) && fetchedUser[key] !== "") {
            setProp.setProp(key, fetchedUser[key], user);
        }
    });
...
const enrichGroupContentCount = (input) => {
    ...
    return search$1.searchGroupContent(options)
        .then((results) => {
        stack.finish(opId);
        return {
...
const enrichGroupMembershipSummary = (input) => {
    ...
    return get.searchGroupUsers(data.group.id, options)
        .then((results) => {
        stack.finish(opId);
        return {
            data: Object.assign(Object.assign({}, data), {
...
const enrichGroupUserMembership = (input) => {
    ...
    return get.getGroup(data.group.id, requestOptions)
        .then((result) => {
        stack.finish(opId);
        return {
            data: Object.assign(Object.assign({}, data), {
                membershipAccess: result.membershipAccess,
                userMembership: getProp.getProp(result, "userMembership.memberType"),
            }),
            stack,
            requestOptions,
        };
...
function computeLinks(group, requestOptions) {
    ...
    return {
        ...
        siteRelativeEntityType: compose.getHubRelativeUrl("Group"),
        workspaceRelative: getRelativeWorkspaceUrl.getRelativeWorkspaceUrl("Group", group.id),
        thumbnail: HubInitiatives.getGroupThumbnailUrl(requestOptions.portal, group, token),
    };
}
...
function computeProps(group, hubGroup, requestOptions) {
    ...
    // thumbnail url
    hubGroup.thumbnailUrl = HubInitiatives.getGroupThumbnailUrl(requestOptions.portal, group, token);
    // Handle Dates
    ...
    hubGroup.type = "Group";
    hubGroup.isDiscussable = utils.isDiscussable(group);
...
function convertGroupToHubGroup(group, requestOptions) {
    const mapper = new PropertyMapper.PropertyMapper(HubInitiatives.getPropertyMap());
...
async function createHubGroup(partialGroup, requestOptions) {
    ...
    hubGroup.typeKeywords = utils.setDiscussableKeyword(hubGroup.typeKeywords, hubGroup.isDiscussable);
    const group = HubInitiatives.convertHubGroupToGroup(hubGroup);
    const opts = {
        group,
        authentication: requestOptions.authentication,
    };
    const result = await remove.createGroup(opts);
    ...
    if (group.protected) {
        result.group.protected = (await remove.protectGroup({
            id: result.group.id,
            authentication: requestOptions.authentication,
        })).success;
    }
...
async function fetchHubGroup(identifier, requestOptions) {
    const group = await get.getGroup(identifier, requestOptions);
...
async function updateHubGroup(hubGroup, requestOptions) {
    ...
    if (group._clearEmptyFields) {
        setProp.setProp("params.clearEmptyFields", true, opts);
    }
    await update.updateGroup(opts);
    return hubGroup;
}
/**
 * @private
...
async function portalSearchGroups(query, options) {
    if (!options.requestOptions) {
        throw new HubError.HubError("portalSearchGroups", "options.requestOptions is required.");
    }
    // Expand the individual predicates in each filter
    query.filters = query.filters.map((filter) => {
        filter.predicates = filter.predicates.map(HubInitiatives.expandPredicate);
        return filter;
    });
    // Serialize the all the groups for portal
    const so = HubInitiatives.serializeQueryForPortal(query);
...
async function searchPortal$1(searchOptions) {
    ...
    const resp = await search$1.searchGroups(searchOptions);
    // create mappable fn that will close
    // over the includes and requestOptions
    const fn = (item) => {
    ...
    return {
        ...
        results,
        hasNext: resp.nextStart > -1,
        next: HubInitiatives.getNextFunction(searchOptions, resp.nextStart, resp.total, searchPortal$1),
        executedQuerySize: HubInitiatives.getKilobyteSizeOfQuery(searchOptions.q),
    };
}
/**
...
function buildSearchOptions(query, options, operation) {
    ...
    if (!options.requestOptions) {
        throw new HubError.HubError(operation, "requestOptions: IHubRequestOptions is required.");
    }
    if (!options.requestOptions.authentication) {
        throw new HubError.HubError(operation, "requestOptions must pass authentication.");
    }
    ...
    // Serialize the all the groups for portal
    const so = HubInitiatives.serializeQueryForPortal(clonedQuery);
    // Array of properties we want to copy from IHubSearchOptions to the ISearchOptions
    const props = [
        "num",
        "sortField",
...
async function searchPortal(searchOptions) {
    ...
    return {
        ...
        results,
        hasNext: resp.nextStart > -1,
        next: HubInitiatives.getNextFunction(searchOptions, resp.nextStart, resp.total, searchPortal),
        executedQuerySize: HubInitiatives.getKilobyteSizeOfQuery(searchOptions.q),
    };
}
/**
...
async function searchCommunity(searchOptions) {
    ...
    return {
        ...
        results,
        hasNext: resp.nextStart > -1,
        next: HubInitiatives.getNextFunction(searchOptions, resp.nextStart, resp.total, searchCommunity),
        executedQuerySize: HubInitiatives.getKilobyteSizeOfQuery(searchOptions.q),
    };
}
/**
...
function formatPredicate(predicate) {
    const formatted = Object.entries(predicate)
        ...
        .filter(([field, value]) => field !== "term" && field !== "bbox" && !HubInitiatives.isNilOrEmptyString(value))
...
function getOgcAggregationQueryParams(query, options) {
    ...
    const token = getProp.getProp(options, "requestOptions.authentication.token");
    return {
        aggregations,
        filter,
        token,
...
function getQueryString(queryParams) {
    const result = Object.entries(queryParams)
        .filter(([_key, value]) => !HubInitiatives.isNilOrEmptyString(value))
...
async function ogcApiRequest(url, queryParams, options) {
    var _a;
    const updatedQueryParams = util.cloneObject(queryParams);
    ...
    if (!response.ok) {
        throw new request$1.RemoteServerError(response.statusText, withQueryString, response.status);
    }
...
async function ogcItemToSearchResult(ogcItem, includes, requestOptions) {
    ...
    const result = await HubInitiatives.itemToSearchResult(pseudoItem, includes, requestOptions);
...
function getBboxQueryParam(query) {
    const bboxPredicate = HubInitiatives.getTopLevelPredicate("bbox", query.filters);
...
function getQQueryParam(query) {
    const qPredicate = HubInitiatives.getTopLevelPredicate("term", query.filters);
...
const processSearchParams = (options, query) => {
    if (!options.requestOptions) {
        throw new HubError.HubError("hubSearchChannels", "options.requestOptions is required");
    }
...
const toHubSearchResults = async (channelsResponse, query, options) => {
    ...
    const itemsAndGroups = await Promise.all(items.map(async (channel) => {
        ...
        const groups = ((_a = options.include) === null || _a === void 0 ? void 0 : _a.includes("groups")) ? await Promise.all(channel.groups.map(async (groupId) => {
            let group;
            try {
                group = await get.getGroup(groupId, options.requestOptions);
            }
            catch (e) {
                group = null;
    ...
    return {
        total,
        results: itemsAndGroups.map(({ channel, groups }) => utils.channelToSearchResult(channel, groups)),
        hasNext: nextStart > -1,
        next: () => {
...
const hubSearchChannels = async (query, options) => {
    ...
    const channelsResponse = await channels.searchChannels(searchOptions);
...
async function eventToSearchResult(event, options) {
    ...
    if ((_a = options.include) === null || _a === void 0 ? void 0 : _a.includes("ownerUser")) {
        ownerUser = await getUser.getUser(Object.assign({ username: event.creator.username }, options.requestOptions));
    }
    const result = {
        access: event.access.toLowerCase(),
        id: event.id,
        location: registrations.getLocationFromEvent(event),
        type: "Event",
        name: event.title,
        ...
        family: "event",
        links: registrations.computeLinks(event),
        tags: event.tags,
...
function processOptions(options) {
    ...
    if (options.sortField === "modified") {
        processedOptions.sortBy = events.EventSort.updatedAt;
    }
    else if (options.sortField === "created") {
        processedOptions.sortBy = events.EventSort.createdAt;
    }
    else if (options.sortField === "title") {
        processedOptions.sortBy = events.EventSort.title;
    }
    else if (options.sortField === "startDate") {
        processedOptions.sortBy = events.EventSort.startDateTime;
    }
    processedOptions.sortOrder =
        options.sortOrder === "desc" ? events.EventSortOrder.desc : events.EventSortOrder.asc;
...
const getOptionalPredicateStringsByKey = (filters, predicateKey) => {
    ...
    const str = predicateValues.filter(util.unique).join(",");
    if (str) {
        return str;
    }
};
...
async function processFilters(filters, requestOptions) {
    ...
    if (notGroupIds.length) {
        const { results } = await search$1.searchGroups(Object.assign({ q: `id:(${notGroupIds.join(" OR ")})`, num: notGroupIds.length }, requestOptions));
        const { notReadGroupIds, notEditGroupIds } = results.reduce((acc, group) => {
            const key = isUpdateGroup.isUpdateGroup(group)
                ? "notEditGroupIds"
                : "notReadGroupIds";
    ...
    processedFilters.status = (status === null || status === void 0 ? void 0 : status.length) ? status
        : [events.EventStatus.PLANNED, events.EventStatus.CANCELED]
...
async function hubSearchEvents(query, options) {
    ...
    const { items, nextStart, total } = await events.getEvents(Object.assign(Object.assign({}, options.requestOptions), { data }));
...
async function eventAttendeeToSearchResult(attendee, options) {
    ...
    const [user, creator] = await Promise.all([
        getUser.getUser(Object.assign({ username: attendee.userId }, options.requestOptions)),
        getUser.getUser(Object.assign({ username: attendee.createdById }, options.requestOptions)),
    ]);
    return {
        ...
        links: {
            ...
            thumbnail: user.thumbnail
                ? HubInitiatives.getUserThumbnailUrl(options.requestOptions.portal, user, (_a = options.requestOptions.authentication) === null || _a === void 0 ? void 0 : _a.token)
                : null,
        },
    };
}
...
function processAttendeeOptions(options) {
    ...
    if (options.sortField === "modified") {
        processedOptions.sortBy = events.RegistrationSort.updatedAt;
    }
    else if (options.sortField === "created") {
        processedOptions.sortBy = events.RegistrationSort.createdAt;
    }
    else if (options.sortField === "username") {
        processedOptions.sortBy = events.RegistrationSort.username;
    }
    else if (options.sortField === "firstName") {
        processedOptions.sortBy = events.RegistrationSort.firstName;
    }
    else if (options.sortField === "lastName") {
        processedOptions.sortBy = events.RegistrationSort.lastName;
    }
    processedOptions.sortOrder =
        options.sortOrder === "desc" ? events.EventSortOrder.desc : events.EventSortOrder.asc;
    return processedOptions;
...
function processAttendeeFilters(query) {
    ...
    processedFilters.type = useElseJoin(getOptionalPredicateStringsByKey(query.filters, "attendanceType"), [events.EventAttendanceType.VIRTUAL, events.EventAttendanceType.IN_PERSON]);
    processedFilters.role = useElseJoin(getOptionalPredicateStringsByKey(query.filters, "role"), [
        events.RegistrationRole.OWNER,
        events.RegistrationRole.ORGANIZER,
        events.RegistrationRole.ATTENDEE,
    ]);
    processedFilters.status = useElseJoin(getOptionalPredicateStringsByKey(query.filters, "status"), [
        events.RegistrationStatus.PENDING,
        events.RegistrationStatus.ACCEPTED,
        events.RegistrationStatus.DECLINED,
        events.RegistrationStatus.BLOCKED,
    ]);
...
async function hubSearchEventAttendees(query, options) {
    ...
    const { items, nextStart, total } = await registrations.getRegistrations(Object.assign(Object.assign({}, options.requestOptions), { data }));
...
async function hubSearch(query, options) {
    // Validate inputs
    if (!query) {
        throw new HubError.HubError("hubSearch", "Query is required.");
    }
    if (!Array.isArray(query.filters)) {
        throw new HubError.HubError("hubSearch", "Query must have a filters array.");
    }
    if (!query.filters.length && !query.collection) {
        throw new HubError.HubError("hubSearch", "Query must contain at least one Filter or a collection.");
    }
    if (!options.requestOptions) {
        throw new HubError.HubError("hubSearch", "requestOptions: IHubRequestOptions is required.");
    }
    ...
    const { requestOptions } = options, remainder = tslib_es6.__rest(options, ["requestOptions"]);
    const formattedOptions = util.cloneObject(remainder);
    formattedOptions.requestOptions = requestOptions;
    formattedOptions.api = getApi(filterType, formattedOptions);
    const fnHash = {
        arcgis: {
            item: HubInitiatives.portalSearchItems,
    ...
    const fn = getProp.getProp(fnHash, `${formattedOptions.api.type}.${filterType}`);
    if (!fn) {
        throw new HubError.HubError(`hubSearch`, `Search via "${filterType}" filter against "${formattedOptions.api.type}" api is not implemented. Please ensure "targetEntity" is defined on the query.`);
    }
    return fn(util.cloneObject(query), formattedOptions);
}

exports.DEFAULT_GROUP = DEFAULT_GROUP;
exports.convertGroupToHubGroup = convertGroupToHubGroup;
exports.convertUserToHubUser = convertUserToHubUser;
...
exports.fetchHubUser = fetchHubUser;
...
exports.updateHubGroup = updateHubGroup;
