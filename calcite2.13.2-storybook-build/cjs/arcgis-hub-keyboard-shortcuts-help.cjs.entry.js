...
const ArcgisHubKeyboardShortcutsHelp = class {
  ...
  renderConfig() {
    ...
    return Object.entries(groupedShortcuts).map(([categoryKey, shortcuts]) => {
      return (index.h("section", { key: categoryKey }, index.h("h2", null, this.intl.t(categoryKey)), index.h("ul", null, shortcuts.map(shortcut => {
        const keyboardKey = shortcut.key || shortcut.code.replace(/^Key/, '').toLowerCase();
        return (index.h("li", { key: shortcut.labelKey }, index.h("div", null, this.intl.t(shortcut.labelKey)), index.h("div", null, shortcut.modifiers.map(modifier => {
          const label = this.mapKeyToLabel(modifier);
          return index.h("span", { class: "modifier-key", key: label }, label);
        }), index.h("span", { class: "modifier-key" }, keyboardKey))));
      }))));
    });
  }
  render() {
    return (index.h(index.Host, { "data-element": "keyboard-shortcuts" }, this.renderConfig()));
  }
};
ArcgisHubKeyboardShortcutsHelp.style = arcgisHubKeyboardShortcutsHelpCss;

exports.arcgis_hub_keyboard_shortcuts_help = ArcgisHubKeyboardShortcutsHelp;
