...
function applyGalleryState(state, serializedState) {
  const updated = util.cloneObject(state);
  // Apply default query
...
function resetFacet(facet) {
  let resetted = util.cloneObject(facet);
...
function resetOptionsBasedFacet(facet) {
  const resetted = util.cloneObject(facet);
...
function applySortState(sortOptions, sortState) {
  const updated = util.cloneObject(sortOptions);
  if (sortState) {
...
function applyFacetState(facet, state) {
  let updated = util.cloneObject(facet);
  switch (facet.display) {
    case 'single-select':
    case 'multi-select':
...
function serializeMapFacetState(facet) {
  return {
    [facet.key]: facet.value && extent.bboxToString(facet.value)
  };
}
/**
...
function processCommonDynamicOptions(facet, intl) {
  let processedOptions = util.cloneObject(facet.options);
  switch (facet.field) {
    case 'type':
      processedOptions = processedOptions.map(opt => {
        ...
        const formattedKey = allCapsTypes.includes(opt.key) ? opt.key.toUpperCase() : titleize.titleize(opt.key);
    ...
    case 'tags':
      processedOptions = processedOptions.map(opt => {
        opt.label = titleize.titleize(opt.label);
        return opt;
      });
      break;
    case 'license':
...
function removeCategoriesOptionCounts(options) {
  const result = util.cloneObject(options);
...
function formatCategoriesOptionLabels(options) {
  ...
  result.forEach(opt => {
    ...
    opt.label = titleize.titleize(pathSegments[pathSegments.length - 1]);
  });
  return result;
}
/**
...
exports.addPathToResults = addPathToResults;
exports.applyGalleryState = applyGalleryState;
exports.convertToQueryString = convertToQueryString;
exports.filterFacetsBasedOnLayout = filterFacetsBasedOnLayout;
exports.getDateRangeFacets = getDateRangeFacets;
exports.getListFacets = getListFacets;
exports.getMapFacets = getMapFacets;
exports.getOptionsBasedFacets = getOptionsBasedFacets;
exports.getTreeFacets = getTreeFacets;
exports.hasMapFacets = hasMapFacets;
exports.isMapFacet = isMapFacet;
exports.isOptionsBasedFacet = isOptionsBasedFacet;
exports.isTreeFacet = isTreeFacet;
exports.isValidDateRange = isValidDateRange;
exports.navigate = navigate;
exports.processCommonDynamicOptions = processCommonDynamicOptions;
exports.removeCountLabel = removeCountLabel;
exports.resetFacet = resetFacet;
exports.serializeGalleryState = serializeGalleryState;
exports.willStateChange = willStateChange;
