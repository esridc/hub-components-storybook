...
const arcgisHubInitiativeOverviewCss = ".sc-arcgis-hub-initiative-overview-h{display:block;height:100%}.initiative-overview__main.sc-arcgis-hub-initiative-overview{border-radius:10px;border:1px solid var(--calcite-color-border-2);display:flex;flex-direction:column;gap:1rem;background-color:var(--calcite-color-foreground-1);padding:1rem}calcite-chip.sc-arcgis-hub-initiative-overview{margin-right:1.5rem}[slot=\"title\"].sc-arcgis-hub-initiative-overview{margin:0px;font-size:var(--calcite-font-size-3);line-height:2rem;font-weight:var(--calcite-font-weight-bold);color:var(--calcite-color-text-1)}.initiative-overview__status-purpose-container.sc-arcgis-hub-initiative-overview{display:flex}calcite-chip.sc-arcgis-hub-initiative-overview,.initiative-overview__purpose.sc-arcgis-hub-initiative-overview{align-self:center}arcgis-hub-map.sc-arcgis-hub-initiative-overview{position:relative;aspect-ratio:200 / 133;height:20rem;width:100%}arcgis-hub-map.sc-arcgis-hub-initiative-overview .initiative-overview__map-overlay.sc-arcgis-hub-initiative-overview{position:absolute;z-index:10;display:flex;height:20rem;width:100%;flex-direction:column;justify-content:center;background-color:#0000004d;color:var(--calcite-color-text-inverse)}.initiative-overview__no-location-message.sc-arcgis-hub-initiative-overview{text-align:center}.initiative-overview__no-purpose-message.sc-arcgis-hub-initiative-overview{font-style:italic;color:var(--calcite-color-text-3)}";

const ArcgisHubInitiativesOverview = class {
  constructor(hostRef) {
  ...
  renderLearnAboutNotice() {
    return (index.h("calcite-notice", { icon: 'lightbulb', kind: 'brand', open: true, width: "full" }, index.h("arcgis-hub-workspace-link", { href: "https://www.esri.com/arcgis-blog/products/arcgis-hub/announcements/introducing-redefined-initiatives-in-arcgis-hub/", iconEnd: "launch", slot: "link", target: "_blank", telemetry: index$2.dist.dictionary.category.navigation.action.external.label.webHelp.details.learnMoreAboutHubInitiatives }, this.intl.t('learnAbout'))));
  }
  renderStatusAndPurpose() {
    return (index.h("div", { class: 'initiative-overview__status-purpose-container' }, index.h("calcite-chip", { appearance: "outline", icon: this.entity.status === 'complete' ? 'check' : '', scale: "l", value: this.entity.status }, this.intl.t(`status.${this.entity.status}`)), index.h("div", { class: 'initiative-overview__purpose' }, this.renderPurpose())));
  }
  renderPurpose() {
    return (this.entity.summary
      ? index.h("div", null, this.entity.summary)
      : index.h("div", { class: 'initiative-overview__no-purpose-message' }, this.intl.t('noPurposeSet')));
  }
  renderMap() {
    return (index.h("arcgis-hub-map", { basemap: 'gray-vector', expand: 1.5, extent: this.extent, graphics: this.graphics, settings: this.mapSettings }, this.hasLocation
      ? null
      : index.h("div", { class: "initiative-overview__map-overlay" }, index.h("div", { class: "initiative-overview__no-location-message" }, this.intl.t('noLocationSet')))));
  }
  renderEditButton() {
  ...
  render() {
    return (index.h(index.Host, { "data-element": "initiative-overview" }, index.h("arcgis-hub-workspace-pane", null, index.h("h1", { slot: "title" }, this.intl.t('overview')), index.h("div", null, index.h("div", { class: 'initiative-overview__main' }, this.renderStatusAndPurpose(), this.renderMap(), this.renderEditButton())), index.h("div", { slot: "side-panel" }, this.renderLearnAboutNotice(), this.shouldShowPageViews && this.renderPageViews(), this.renderMetadata()))));
  }
  static get assetsDirs() { return ["locales"]; }
  get element() { return index.getElement(this); }
};
ArcgisHubInitiativesOverview.style = arcgisHubInitiativeOverviewCss;

exports.arcgis_hub_initiative_overview = ArcgisHubInitiativesOverview;
