...
async function unshareEntitiesToGroups(groups, entities, context) {
  const nested = await Promise.all(groups.map(async (g) => {
    ...
    return await Promise.all(entities.map(async (e) => {
      ...
      try {
        await unshareEntityWithGroups.unshareEntityWithGroups(e, [g.id], context);
      }
      catch (_ex) {
        result.status = "fail";
      }
...
async function shareEntitiesToGroups(groups, entities, context) {
  const nested = await Promise.all(groups.map(async (g) => {
    ...
    return await Promise.all(entities.map(async (e) => {
      const result = { id: e.id, groupId: g.id, status: "success" };
      try {
        await unshareEntityWithGroups.shareEntityWithGroups(e, [g.id], context);
      }
      catch (_ex) {
        result.status = "fail";
      }
  ...
  const results = nested.flat();
  const entityType = getTypeFromEntity.getTypeFromEntity(entities[0]);
...
exports.shareEntitiesToGroups = shareEntitiesToGroups;
exports.unshareEntitiesToGroups = unshareEntitiesToGroups;
