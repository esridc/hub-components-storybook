...
function arrayToObject(arr, key) {
    return arr.reduce((hash, entry) => {
        hash[getProp.getProp(entry, key)] = entry;
        return hash;
    }, {});
}
/**
...
function findBy(arr, prop, value) {
    ...
    const res = arr.reduce((acc, entry) => {
        if (getProp.getProp(entry, prop) === value) {
            acc = entry;
        }
        return acc;
    }, null);
...
function uniqueBy(arr, prop) {
    return arr.reduce((acc, entry) => {
        const nameMatches = (e) => getProp.getProp(e, prop) === getProp.getProp(entry, prop);
...
function filterBy(arr, prop, val) {
    return arr.filter((entry) => getProp.getProp(entry, prop) === val);
}
/**
