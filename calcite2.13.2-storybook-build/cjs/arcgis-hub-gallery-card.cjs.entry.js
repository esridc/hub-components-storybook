...
const ArcgisHubGalleryCard = class {
  ...
  get _imageType() {
    ...
    const result = { Thumbnails: interfaces.IMAGE_TYPES.thumbnail, Icons: interfaces.IMAGE_TYPES.icon }[this.imageType];
    return result || interfaces.IMAGE_TYPES.thumbnail;
  }
  get _isManualMode() {
  ...
  async _fetchCatalogData() {
    ...
    if (catalogs.length && this.mode === MODE.dynamic) {
      ...
      const catalogItemDatas = await Promise.all(catalogs.map(catalogItemId => get.getItemData(catalogItemId, opts)));
  ...
  async getState() {
    const { element } = this;
    const state = await shareableUtils.getCardState(element);
  ...
  get scope() {
    ...
    if (!this._isManualMode && catalogData) {
      filters = [...filters, ...util.cloneObject(catalogData)];
    }
    const filter = {
      operation: 'AND',
      predicates: []
    ...
    if (filter.predicates.length) {
      filters = util.maybePush(filter, filters);
    }
    filters = util.maybePush(typeFilters, filters);
    return {
      targetEntity: 'item',
      filters
    };
  ...
  get collection() {
    ...
    this._oldCollection = util.cloneObject(collection);
    return collection;
  }
  get shouldQuery() {
  ...
  _renderGallery() {
    if (this.shouldQuery) {
      ...
      return (index.h("arcgis-hub-help-state", { icon: "file-magnifying-glass" }, index.h("h3", { "aria-live": "polite", role: "status", slot: "message" }, this.intl.t('noResults'))));
    }
  }
  render() {
    return (index.h(index.Host, { "data-element": "gallery-card" }, index.h(shareable.Shareable, { context: this, showShareUi: this.hasResults || this.showEmptyState }, this._renderGallery())));
  }
...
exports.arcgis_hub_gallery_card = ArcgisHubGalleryCard;
