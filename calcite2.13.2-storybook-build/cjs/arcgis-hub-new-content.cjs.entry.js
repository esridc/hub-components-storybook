...
const ArcgisHubNewContent = class {
  constructor(hostRef) {
    ...
    this.arcgisHubNewContentSuccess = index.createEvent(this, "arcgisHubNewContentSuccess", 7);
    this.arcgisHubNewContentError = index.createEvent(this, "arcgisHubNewContentError", 7);
    ...
    this.handleResourceLinkClick = (evt) => {
      ...
      const telemetry = index$1.dist.dictionary.category.navigation.action.new.label.content.details[key];
    ...
    this.handleEntitySelected = () => {
      ...
      if (this.selectedEntity.href) {
        index$1.dist.dictionary.category.navigation.action.new.label.content.details[(_a = this.selectedEntity) === null || _a === void 0 ? void 0 : _a.key]
          ? this.hubTelemetry.emit(index$1.dist.dictionary.category.navigation.action.new.label.content.details[(_b = this.selectedEntity) === null || _b === void 0 ? void 0 : _b.key])
          : this.hubTelemetry.emit(index$1.dist.dictionary.category.navigation.action.new.label.content);
      }
      else {
    ...
    this.toggleForm = async (isOpen) => {
      ...
      this.selectedEntity && this.hubTelemetry.emit(isOpen
        ? index$1.dist.dictionary.category.interaction.action.open.label.modal.details[`create${util.capitalize((_a = this.selectedEntity) === null || _a === void 0 ? void 0 : _a.key)}`]
        : index$1.dist.dictionary.category.interaction.action.close.label.modal.details[`create${util.capitalize((_b = this.selectedEntity) === null || _b === void 0 ? void 0 : _b.key)}`]);
    };
  ...
  renderNewDropdownButton(entityConfigs, resourceLinks) {
    let button;
    if (this.isMobile) {
      button = (index.h("calcite-button", { appearance: this.buttonAppearance, kind: calcite.buttonColorToKind(this.buttonColor), onClick: this.onOpenMobileDropdownButtonClick, scale: this.buttonScale }, index.h("calcite-icon", { icon: "plus", scale: this.buttonScale })));
    }
    else {
      button = this.isSingleEntity && !this.renderAsDropdown
        ? (index.h("arcgis-ref-tooltip", { overlayPositioning: 'fixed', placement: "bottom", text: entityConfigs[0].disabled && entityConfigs[0].disabledTooltip || '' }, index.h("calcite-button", { appearance: this.buttonAppearance, href: entityConfigs[0].href, kind: calcite.buttonColorToKind(this.buttonColor), onClick: entityConfigs[0].disabled ? null : this.handleEntitySelected, round: true, scale: this.buttonScale, shouldBeDisabled: entityConfigs[0].disabled }, entityConfigs[0].label || this.intl.t(`${entityConfigs[0].key}.createLabel`))))
        : this.renderDropdown(entityConfigs, resourceLinks);
    }
    return button;
  }
  renderDropdown(entityConfigs, resourceLinks) {
    return (index.h("calcite-dropdown", { ref: this.handleCalciteDropdownRef }, index.h("calcite-button", { appearance: this.buttonAppearance, "icon-end": "caret-down", kind: calcite.buttonColorToKind(this.buttonColor), scale: this.buttonScale, slot: "trigger" }, this.intl.t('new')), this.renderDropdownContent(entityConfigs, resourceLinks)));
  }
  /**
  ...
  renderMobileDropdown(entityConfigs, resourceLinks) {
    return (index.h("calcite-modal", { onCalciteModalClose: this.onCloseMobileDropdownButttonClick, open: this.isMobileDropdownOpen }, index.h("div", { slot: "header" }, this.intl.t('new')), index.h("div", { slot: "content" }, this.renderDropdownContent(entityConfigs, resourceLinks))));
  }
  renderDropdownContent(entityConfigs, resourceLinks) {
    return (index.h(index.Fragment, null, this.renderEntityItems(entityConfigs), this.hasResourceLinks && this.renderResourceLinks(resourceLinks)));
  }
  renderEntityItems(entityConfigs) {
    return (index.h("calcite-dropdown-group", { class: { "hub-new-content_dropdown-group": true, "mobile": this.isMobile }, "selection-mode": "none" }, entityConfigs.map((entity, idx) => this.renderEntityItem(entity, idx))));
  }
  renderEntityItem(entity, index$1) {
    return (index.h("calcite-dropdown-item", { "data-index": index$1, "data-value": entity.key, href: entity.href, key: entity.key, onCalciteDropdownItemSelect: this.handleDropdownSelect }, index.h("div", { class: "hub-new-content_dropdown-item" }, index.h("div", { class: "hub-new-content_dropdown-item--left" }, index.h("div", { class: "hub-new-content_dropdown-item-icon" }, index.h("calcite-icon", { icon: this.getIcon(entity), scale: "l" })), index.h("div", { class: "hub-new-content_dropdown-item-content" }, index.h("div", { class: "hub-new-content_dropdown-item-title" }, entity.label || this.intl.t(`${entity.key}.label`)), index.h("div", { class: "hub-new-content_dropdown-item-description" }, entity.description || this.intl.t(`${entity.key}.description`)))))));
  }
  renderResourceLinks(resourceLinks) {
    return (index.h("calcite-dropdown-group", { class: { "hub-new-content_dropdown-group": true, "mobile": this.isMobile }, "selection-mode": "none" }, resourceLinks.map(resourceLink => this.renderResourceLink(resourceLink))));
  }
  renderResourceLink(resourceLink) {
    return (index.h("calcite-dropdown-item", { "data-value": resourceLink.key, href: resourceLink.href, key: resourceLink.label, onClick: this.handleResourceLinkClick }, index.h("calcite-link", null, resourceLink.label)));
  }
  renderFormHeader() {
    return (index.h("div", { class: "new-content-modal__header" }, index.h("div", { class: "new-content-modal__header-icon" }, index.h("calcite-icon", { icon: this.getIcon(this.selectedEntity), scale: "l" })), index.h("h2", null, this.formTitle), index.h("span", null, this.formDescription)));
  }
  /**
  ...
  renderForm() {
    var _a;
    return (index.h("arcgis-hub-entity-editor", { editorContext: this.editorContext, editorType: this.editorType, entity: (_a = this.selectedEntity) === null || _a === void 0 ? void 0 : _a.defaults, isOpen: this.isFormOpen, layout: "modal", onArcgisHubEntityEditorSaved: this.handleEntityEditorSaved, onHubTelemetry: this.handleHubTelemetry }, 
    ...
    // when the form opens
    this.isFormModalOpen && (index.h(index.Fragment, null, index.h("div", { slot: "header" }, this.formTitle), index.h("div", { slot: "form-start" }, this.renderFormHeader())))));
  }
  render() {
    ...
    return (index.h(index.Host, { "data-element": dataElement }, !!this.entityConfigs.length && this.renderNewDropdownButton(this.entityConfigs, this.resourceLinks), this.isFormOpen && this.renderForm(), this.isMobile && this.renderMobileDropdown(this.entityConfigs, this.resourceLinks)));
  }
...
exports.arcgis_hub_new_content = ArcgisHubNewContent;
