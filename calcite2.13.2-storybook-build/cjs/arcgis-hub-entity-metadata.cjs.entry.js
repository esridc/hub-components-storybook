...
const arcgisHubEntityMetadataCss = ":host{display:block}.metadata-list-header{font-size:1.125rem;font-weight:600}.entity-metadata-list{list-style:none;padding:0}.metadata-item{margin-top:1.25rem;display:flex;align-items:center;gap:1rem}.metadata-item-text-column{display:flex;flex-direction:column;gap:0.25rem}.metadata-item-title{font-size:1rem;line-height:1rem;font-weight:600}.metadata-item-description{font-size:1rem;line-height:1rem;font-weight:400}";
...
const ArcgisHubEntityMetadata = class {
  ...
  get accessMetadataItem() {
    ...
    return !!access
      ? {
        icon: getAccessIcon.getAccessIcon(access),
  ...
  get typeMetadataItem() {
    ...
    let title = this.intl.t(`type.${util.camelize(type)}`);
    ...
    return !!type
      ? {
        icon: compose.getContentTypeIcon(type),
        title: title,
        description: {
          value: util.capitalize(type)
        },
        type: 'type'
      }
      : undefined;
  ...
  get licenseMetadataItem() {
    ...
    const structuredLicense = getStructuredLicense.getStructuredLicense(this.entity.licenseInfo);
    let title;
    let description;
  ...
  get sizeMetadataItem() {
    ...
    return !!size
      ? {
        title: mediaGalleryUtils.formatBytes(size),
        description: {
  ...
  renderIcon(metadata) {
    ...
    return metadata.icon === 'owner'
      ? index.h("calcite-avatar", { scale: 's', username: (_a = this.entity) === null || _a === void 0 ? void 0 : _a.owner })
      : index.h("calcite-icon", { icon: metadata === null || metadata === void 0 ? void 0 : metadata.icon, scale: "m" });
  }
  /**
  ...
  renderDescription(metadata) {
    ...
    if (!(metadata === null || metadata === void 0 ? void 0 : metadata.description)) {
      ...
      return index.h("calcite-link", { class: `metadata-item-description ${metadata === null || metadata === void 0 ? void 0 : metadata.type}`, href: metadata.description.link.url, iconEnd: metadata.description.link.iconEnd, iconStart: metadata.description.link.iconStart, target: metadata.description.link.target }, metadata.description.value);
    }
      ...
      return index.h(index.Fragment, null, index.h("calcite-link", { class: `metadata-item-description ${metadata === null || metadata === void 0 ? void 0 : metadata.type}`, onClick: linkOnClick }, metadata.description.value), index.h("arcgis-wormhole", null, index.h("calcite-dialog", { heading: metadata.description.modal.title, modal: true, onCalciteDialogClose: onCalciteDialogClose, open: this.openModal === metadata.description.modal.id }, metadata.description.modal.message && index.h("div", { innerHTML: metadata.description.modal.message }))));
    }
    else {
      // return plaintext
      // ex: any item with a description that is just text
      return index.h("span", { class: `metadata-item-description ${metadata === null || metadata === void 0 ? void 0 : metadata.type}` }, metadata.description.value);
    }
  }
  /**
  ...
  renderTitle(metadata) {
    return index.h("div", { class: `metadata-item-title ${metadata === null || metadata === void 0 ? void 0 : metadata.type}` }, metadata.title);
  }
  /**
  ...
  renderMetadataItem(metadata, id) {
    ...
    return (index.h(index.Fragment, null, metadata.tooltip &&
      index.h("calcite-tooltip", { label: this.intl.t('tooltip.label'), "reference-element": `metadata-item-${id}` }, index.h("span", null, metadata.tooltip)), index.h("li", { class: "metadata-item", "data-test": `metadata-item-${id}`, key: id }, this.renderIcon(metadata), index.h("div", { class: "metadata-item-text-column" }, this.renderTitle(metadata), this.renderDescription(metadata)))));
  }
  /**
   ...
   */
  renderFooter() {
    return index.h(index.Fragment, null, this.hasMapFooterSlot && this.renderMetadataItem({
      title: this.intl.t('relevantArea.title'),
      icon: "pin"
    }, "map-footer"), index.h("slot", { name: "map-footer" }));
  }
  render() {
    ...
    return index.h(index.Host, { "data-element": 'entity-metadata' }, this.listHeader && index.h(HeadingTag, { class: "metadata-list-header" }, this.listHeader), index.h("ul", { class: "entity-metadata-list" }, index.h("slot", null), metadataItems.map((metadataItem, index) => this.renderMetadataItem(metadataItem, index)), this.renderFooter()));
  }
...
exports.arcgis_hub_entity_metadata = ArcgisHubEntityMetadata;
