...
const HomeViewModel = require('@arcgis/core/widgets/Home/HomeViewModel.js');
const arcgis = require('./arcgis-492079b8.js');
const LayerList = require('@arcgis/core/widgets/LayerList.js');
const getProp = require('./get-prop-4bd8fc1a.js');
const Legend = require('@arcgis/core/widgets/Legend.js');
...
const HomeViewModel__default = /*#__PURE__*/_interopDefaultLegacy(HomeViewModel);
const LayerList__default = /*#__PURE__*/_interopDefaultLegacy(LayerList);
const Legend__default = /*#__PURE__*/_interopDefaultLegacy(Legend);
...
const ArcgisHubEmbed = class {
  ...
  renderIframe() {
    return this.src && index.h("iframe", { allow: this._allow, class: {
  ...
  render() {
    return (index.h(index.Host, { "data-element": "embed" }, this.renderIframe()));
  }
};
...
const ArcgisHubMapWidgetHome = class {
  constructor(hostRef) {
    ...
    context.bind(this, 'zoomToInitialExtent', 'updateState');
  }
  async componentWillLoad() {
    const { el } = this;
    this.intl = await intlManager.intlManager.loadIntlForComponent(el);
    const [intl] = await Promise.all([
      intlManager.intlManager.loadIntlForComponent(el),
  ...
  connectWatch() {
    ...
    if (view && homeViewModel) {
      handles.push(reactiveUtils_js.watch(() => homeViewModel.state, this.updateState));
    }
  }
  removeWatch() {
    const { handles } = this;
  ...
  async connectViewModel() {
    ...
    if (view) {
      await view.when();
      this.homeViewModel = new HomeViewModel__default['default']({
        view: view
      });
  ...
  zoomToInitialExtent() {
    ...
    if (homeViewModel) {
      homeViewModel.go();
      this.hubTelemetry.emit(Object.assign(Object.assign({}, index$1.dist.dictionary.category.interaction.action.zoom.label.to), { details: "Home" }));
    }
  }
  /**
  ...
  render() {
    ...
    return (index.h(index.Host, { "data-element": "map-widget-home" }, index.h("arcgis-hub-map-widget-generic", { disabled: disabled, icon: 'home', onClick: this.zoomToInitialExtent, scale: scale, text: this.intl.t('home') })));
  }
...
const ArcgisHubMapWidgetLayerList = class {
  constructor(hostRef) {
    ...
    context.bind(this, 'toggleClosed', 'setPanelHeight', 'handleSetPanelRef', 'handleSetWidgetRef');
  }
  async componentWillLoad() {
    const { el } = this;
  ...
  emitEventOnActiveChange(active) {
    const details = active
      ? index$1.dist.dictionary.category.interaction.action.open.label.panel.details.layerList
      : index$1.dist.dictionary.category.interaction.action.close.label.panel.details.layerList;
  ...
  async addWidget() {
    ...
    if (view && container && !layerList) {
      await view.when();
      this.layerList = new LayerList__default['default']({
        view: view,
      ...
      this.layerList.on("trigger-action", (event) => {
        ...
        if (id === "full-extent") {
          ...
          const itemExtent = getProp.getProp(layer, "portalItem.extent");
          const layerExtent = getProp.getProp(layer, "fullExtent");
  ...
  render() {
    ...
    return (index.h(index.Host, { "data-element": "map-widget-layer-list", style: styles }, index.h("calcite-panel", { class: positionClass, closable: true, closed: closed, heading: heading, messageOverrides: this._messageOverrides, ref: this.handleSetPanelRef }), index.h("arcgis-hub-map-widget-generic", { active: active, icon: 'layers', onClick: this.toggleClosed, ref: this.handleSetWidgetRef, scale: scale, text: active ? textClose : textOpen })));
  }
  static get assetsDirs() { return ["locales"]; }
  get el() { return index.getElement(this); }
  static get watchers() { return {
...
const ArcgisHubMapWidgetLegend = class {
  ...
  emitEventOnActiveChange(active) {
    const details = active
      ? index$1.dist.dictionary.category.interaction.action.open.label.panel.details.legend
      : index$1.dist.dictionary.category.interaction.action.close.label.panel.details.legend;
    this.hubTelemetry.emit(details);
  }
  handleCalcitePanelDismissedChange(event) {
    event.stopPropagation();
  ...
  async addWidget() {
    ...
    if (view && container && !legend) {
      await view.when();
      this.legend = new Legend__default['default']({
        view: view,
        container
      });
      reactiveUtils_js.watch(() => view.size, this.setPanelHeight);
...
exports.arcgis_hub_embed = ArcgisHubEmbed;
exports.arcgis_hub_map_widget_home = ArcgisHubMapWidgetHome;
exports.arcgis_hub_map_widget_layer_list = ArcgisHubMapWidgetLayerList;
exports.arcgis_hub_map_widget_legend = ArcgisHubMapWidgetLegend;
