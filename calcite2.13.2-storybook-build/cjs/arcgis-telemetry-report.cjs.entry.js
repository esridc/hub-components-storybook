...
const ArcgisTelemetryReport = class {
  ...
  get _dimensionFilters() {
    ...
    if (this.isCustomEvent) {
      customDimensionFilters = telemetryReportingClient_esm.buildSimpleDimensionFilters(this.telemetryEvent);
    }
    const result = [
  ...
  get _dataLabel() {
    ...
    if (this.dataLabel) {
      ...
      const dataLabelKey = (_a = Object.entries(telemetryReportingClient_esm.COMMON_METRICS).find(([_, value]) => value === this.getMetric(this.telemetryEvent))) === null || _a === void 0 ? void 0 : _a[0];
      return this.intl.t(dataLabelKey);
    }
  }
  getMetric(telemetryEvent) {
    const telemetryEventToMetricMap = {
      [interfaces.COMMON_TELEMETRY.sessions]: telemetryReportingClient_esm.COMMON_METRICS.sessions,
      [interfaces.COMMON_TELEMETRY.referrers]: telemetryReportingClient_esm.COMMON_METRICS['page-views'],
      [interfaces.COMMON_TELEMETRY['page-views']]: telemetryReportingClient_esm.COMMON_METRICS['page-views'],
      [interfaces.COMMON_TELEMETRY['session-activity']]: telemetryReportingClient_esm.COMMON_METRICS['session-activity']
    };
    return !this.isCustomEvent && telemetryEventToMetricMap[telemetryEvent] || telemetryReportingClient_esm.COMMON_METRICS.custom;
  }
  getDimensions(telemetryEvent) {
    const telemetryEventToDimensionsMap = {
      [interfaces.COMMON_TELEMETRY.referrers]: 'referrer',
    };
    ...
    if (isValid) {
      try {
        ...
        const reportResults = await Promise.all([
          telemetryReportingClient_esm.getTelemetryReport(requestParams, requestOptions),
          this.shouldAggregateData ? telemetryReportingClient_esm.getTelemetryReport(Object.assign(Object.assign({}, requestParams), { orderBy: undefined, timeDimension: undefined }), requestOptions) : undefined
        ]);
  ...
  render() {
    ...
    return (index.h(index.Host, { "data-element": "telemetry-report" }, index.h(ReportComponent, Object.assign({}, this.childComponentProps))));
  }
