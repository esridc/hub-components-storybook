...
const types$1 = require('./types-03e19572.js');
...
const getGroupCatalogs = (context, i18nScope, catalogNames) => {
  ...
  const catalogs = _catalogNames.map((name) => {
    ...
    const catalog = wellKnownCatalog.getWellKnownCatalog(i18nScope, name, "group", opts);
    return catalog;
  });
  return catalogs;
};
...
const getItemTargetEntityTypeItems = (intl) => {
  ...
  const comboboxItems = getAgoTypes(rawAgoItemTypes).reduce((acc, type) => {
    ...
    if (family && family !== "event") {
      ...
      if (isNil.isNil(familyToIndex[family])) {
...
const ArcgisHubPredicatesBuilder = class {
  constructor(hostRef) {
    ...
    this.arcgisHubPredicatesBuilderChange = index.createEvent(this, "arcgisHubPredicatesBuilderChange", 7);
    /**
    ...
    this.handlePredicatesEditorChange = async (evt) => {
      ...
      evt.stopPropagation();
      const _predicate = index$1.mergeDeep(util.cloneObject(this._predicates[0]), util.cloneObject(values));
      ...
      const isComplete = ["property", "operator", "value"].every((key) => {
        ...
        return hasField && !isFieldEmpty.isFieldEmpty(_predicate[key]);
      });
  ...
  get _defaultPredicateProperties() {
    ...
    switch (this.targetEntity) {
      case "item":
        properties = types$1.ITEM_PREDICATE_PROPERTIES;
        break;
      case "event":
        properties = types$1.EVENT_PREDICATE_PROPERTIES;
        break;
    }
    return properties;
  }
  ...
  async buildUiSchema() {
    ...
    this._uiSchema = interpolateTranslations.interpolateTranslations(this._intl, uiSchema);
  }
  ;
  /**
  ...
  */
  ...
  renderLoadingState() {
    if (this._isLoading || !this._schema || !this._uiSchema) {
      return index.h("arcgis-skeleton-loader", { active: true, rows: 3, showHeading: false });
    }
  }
  renderEditor() {
    if (!this._isLoading && this._schema && this._uiSchema) {
      return (index.h("arcgis-configuration-editor", { onArcgisConfigurationEditorChange: this.handlePredicatesEditorChange, schema: this._schema, t: this.translationFunc, uiSchema: this._uiSchema, values: this._predicates[0] }));
    }
  }
  render() {
    return (index.h(index.Host, { "data-element": "predicates-builder" }, this.renderLoadingState(), this.renderEditor()));
  }
...
exports.arcgis_hub_predicates_builder = ArcgisHubPredicatesBuilder;
