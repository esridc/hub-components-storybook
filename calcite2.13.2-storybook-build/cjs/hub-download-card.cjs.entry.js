...
var getUserContent = function (requestOptions) {
    ...
    return get.determineOwner(requestOptions)
        .then(function (owner) { return getPortalUrl.getPortalUrl(requestOptions) + "/content/users/" + owner + suffix; })
        .then(function (url) { return request.request(url, {
        httpMethod: "GET",
...
var exportItem = function (requestOptions) {
    ...
    return get.determineOwner(requestOptions)
        .then(function (owner) { return getPortalUrl.getPortalUrl(requestOptions) + "/content/users/" + owner + "/export"; })
        .then(function (url) { return request.request(url, {
        httpMethod: 'POST',
        authentication: authentication,
        params: {
...
function getExportsFolderId(authentication) {
    return getUserContent({ authentication })
        ...
        return create.createFolder({ authentication, title: "item-exports" });
    })
        .then((response) => {
...
function exportSuccessHandler(params) {
    ...
    const { itemId, layerId } = slugs.parseDatasetId(datasetId);
    const typeKeywords = [
        buildExistingExportsPortalQuery.getExportItemTypeKeyword(itemId),
        buildExistingExportsPortalQuery.getExportLayerTypeKeyword(layerId),
        `modified:${exportCreated}`,
        buildExistingExportsPortalQuery.getSpatialRefTypeKeyword(spatialRefId),
    ];
...
function isDownloadEnabled(item, format) {
    ...
    const isItemLevelDownloadEnabled = getProp.getProp(item, "properties.downloadsConfig.enabled") !== false;
    const formats = getProp.getProp(item, "properties.downloadsConfig.formats") || {};
...
function portalRequestDownloadMetadata(params) {
    ...
    let itemType;
    let fetchedItem;
    return get.getItem(itemId, { authentication, portal })
        .then((item) => {
        ...
        return search.searchItems({
            q: buildExistingExportsPortalQuery.buildExistingExportsPortalQuery(itemId, {
                layerId,
...
function fetchCacheSearchMetadata(params) {
    ...
    return retryWithoutAuthOnFail(getService.getService({ url, authentication, portal }))
        .then((response) => {
        const layers = response.layers || [];
        const promises = layers.map((layer) => {
            return retryWithoutAuthOnFail(getLayer.getLayer({ url: `${url}/${layer.id}`, authentication, portal }));
        });
...
const HubDownloadCard = class {
  constructor(hostRef) {
    ...
    this.hubDownloadCardSuccess = index.createEvent(this, "hubDownloadCardSuccess", 7);
    ...
    context.bind(this, 'exportDatasetComplete', 'exportDatasetError', 'exportPollingError', 'download', 'downloadKeyDown', 'exportDataset', 'exportDatasetKeyDown');
  }
  ...
  buildDownloadTelemetry({ isSuccess = true, isCached = true } = {}) {
    return Object.assign(Object.assign({}, isCached ? index$1.dist.dictionary.category.interaction.action.download.details.cache : index$1.dist.dictionary.category.interaction.action.download.details.export), { label: this.format, response: isSuccess ? index$1.dist.constants.response.SUCCESS : index$1.dist.constants.response.FAILURE });
  }
  resetUndefinedProps() {
  ...
  setAuthentication() {
    if (this.username && this.token) {
      this.authentication = new UserSession.UserSession({
  ...
  transferFile(url) {
    ...
    return urls.redirectToExternalUrl(url);
  }
  async exportDataset() {
  ...
  renderNotice() {
    ...
    return index.h("hub-download-notice", { "api-error": error, "cannot-export": !userCanExport(this.target, this.username), "export-requested": exportRequested, "file-status": modifiedStatus });
  }
  exportInProgress() {
  ...
  renderFileDescription() {
    ...
    if (lastModified) {
      if (this.intl.direction === 'rtl') {
        fileDescription.push(index.h("dt", null, this.dateTimeFormatter.format(new Date(lastModified))), index.h("dd", { class: "rtl" }, this.labels.fileCreated));
      }
      else {
        fileDescription.push(index.h("dt", { class: "ltr" }, this.labels.fileCreated), index.h("dd", null, this.dateTimeFormatter.format(new Date(lastModified))));
      }
    }
    if (this.contentLengthString) {
      if (this.intl.direction === 'rtl') {
        fileDescription.push(index.h("dt", null, this.contentLengthString), index.h("dd", { class: "rtl" }, this.labels.fileSize));
      }
      else {
        fileDescription.push(index.h("dt", { class: "ltr" }, this.labels.fileSize), index.h("dd", null, this.contentLengthString));
      }
    }
  ...
  renderDownloadControl() {
    if (this.shouldUseDownloadButton()) {
      return index.h("calcite-button", { appearance: "solid", color: "blue", disabled: this.shouldDisableDownload(), "icon-position": "start", onClick: this.download, scale: "m", width: "full" }, this.labels.buttonDownload);
    }
    return (index.h("calcite-dropdown", { open: false, placement: "top", scale: "m", type: "click" }, index.h("calcite-button", { appearance: "solid", color: "blue", "icon-end": "caretUp", scale: "m", slot: "trigger" }, this.labels.buttonOptions), index.h("calcite-dropdown-group", { "selection-mode": "none" }, index.h("calcite-dropdown-item", { hidden: this.exportInProgress(), onClick: this.exportDataset, onKeyDown: this.exportDatasetKeyDown, role: "menuitem", "selection-mode": "none", tabindex: "0" }, index.h("span", null, this.labels.menuRequest)), index.h("calcite-dropdown-item", { onClick: this.download, onKeyDown: this.downloadKeyDown, role: "menuitem", "selection-mode": "none", tabindex: "0" }, index.h("span", null, this.intl.t('menuDownload', { createdDate: this.dateTimeFormatter.format(new Date(this.metadata.lastModified)) }))))));
  }
  ...
  render() {
    return (index.h(index.Host, { "data-element": "download-card" }, index.h("calcite-card", { dir: this.intl.direction }, index.h("h3", { slot: "title" }, this.name), index.h("dl", { slot: "subtitle" }, this.renderFileDescription()), index.h("div", { slot: "footer-start" }, this.renderNotice(), this.renderDownloadControl()))));
  }
...
exports.hub_download_card = HubDownloadCard;
