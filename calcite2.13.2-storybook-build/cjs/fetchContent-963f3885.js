...
function getFeature(requestOptions) {
    var url = cleanUrl.cleanUrl(requestOptions.url) + "/" + requestOptions.id;
    // default to a GET request
    var options = _enrichments.__assign({ httpMethod: "GET" }, requestOptions);
...
function queryFeatures(requestOptions) {
    var queryOptions = appendCustomParams.appendCustomParams(requestOptions, [
        "where",
        "objectIds",
        "relationParam",
        "time",
        ...
        httpMethod: "GET",
        params: _enrichments.__assign({ 
            // set default query parameters
            where: "1=1", outFields: "*" }, requestOptions.params)
    });
    return request.request(cleanUrl.cleanUrl(requestOptions.url) + "/query", queryOptions);
}
...
const shouldFetchData = (item) => {
    ...
    return compose.includes(dataFamilies, family) || compose.includes(dataTypes, type);
};
/**
...
const getContentEnrichments = (item) => {
    ...
    return index.isMapOrFeatureServerUrl(item.url)
...
const getHubEnrichmentsOptions = (requestOptions, slug) => {
    const opts = util.cloneObject(requestOptions);
...
const getDatasetEnrichments = (dataset) => {
    const { itemId, layerId: layerIdString } = slugs.parseDatasetId(dataset.id);
...
const fetchHubEnrichmentsBySlug = async (slug, requestOptions) => {
    ...
    const response = await request$1.hubApiRequest(`/datasets`, getHubEnrichmentsOptions(requestOptions, slug));
...
const fetchHubEnrichmentsById = async (hubId, requestOptions) => {
    try {
        const response = await request$1.hubApiRequest(`/datasets/${hubId}`, getHubEnrichmentsOptions(requestOptions));
        ...
        return { errors: _enrichments.getEnrichmentErrors(e) };
    }
};
...
const maybeFetchLayerEnrichments = async (itemAndEnrichments, options) => {
    ...
    let layer = layers && compose.getItemLayer(item, layers, options && options.layerId);
    ...
    if (layer && !layer.type) {
        const layerUrl = helpers.parseServiceUrl(item.url) + "/" + layer.id;
        const getLayerOptions = Object.assign({ url: layerUrl }, options); // works whether options is defined or not
        layer = await getLayer.getLayer(getLayerOptions);
...
const fetchItemAndEnrichments = async (itemId, options) => {
    // fetch the item
    const item = await get.getItem(itemId, options);
    ...
    item.url = compose.getProxyUrl(item, options) || item.url;
    // fetch the enrichments
    const enrichmentsToFetch = (options === null || options === void 0 ? void 0 : options.enrichments) || getContentEnrichments(item);
    const enrichments = await _enrichments.fetchItemEnrichments(item, enrichmentsToFetch, options);
...
const fetchContentById = async (hubId, options) => {
    ...
    const { itemId } = slugs.parseDatasetId(hubId);
    const _a = await fetchItemAndEnrichments(itemId, options), { item } = _a, itemEnrichments = tslib_es6.__rest(_a, ["item"]);
    ...
    const hubEnrichments = compose.canUseHubApiForItem(item, options)
    ...
    return compose.composeContent(item, Object.assign(Object.assign(Object.assign({ requestOptions: options }, itemEnrichments), hubEnrichments), { 
        // prefer specified layer id if any
        layerId: util.isNil(specifiedLayerId) ? layerId : specifiedLayerId, 
        // merge error arrays
...
const fetchContentBySlug = async (fullyQualifiedSlug, options) => {
    ...
    // did the caller request a specific layer
    const specifiedLayerId = options && options.layerId;
    if (!util.isNil(specifiedLayerId) && specifiedLayerId !== layerId) {
    ...
    return compose.composeContent(item, Object.assign(Object.assign(Object.assign({ requestOptions: options }, itemEnrichments), hubEnrichments), { layerId, 
...
const fetchContent = async (identifier, options) => {
    const content = slugs.isSlug(identifier)
        ? await fetchContentBySlug(slugs.addContextToSlug(identifier, options === null || options === void 0 ? void 0 : options.siteOrgKey), options)
    ...
    content.recordCount =
        canQuery && (util.isNil(content.recordCount) || content.viewDefinition)
