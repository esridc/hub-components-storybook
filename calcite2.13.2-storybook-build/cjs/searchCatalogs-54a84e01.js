...
async function searchCatalogs(catalogs, query, options, context) {
    ...
    const instances = catalogs.map((catalog) => Catalog.Catalog.fromJson(catalog, context));
    // Behavior:
    ...
    const promises = instances.map(async (cat) => {
        ...
        const collectionTargets = mapBy.mapBy("targetEntity", cat.collections).filter(util.unique);
