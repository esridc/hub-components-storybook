...
function getOpsDashboardUrl(id, context) {
  ...
  const portalUrl = getProp(context, 'portalUrl') || DEFAULT_OPTIONS.portalUrl;
...
function getExperienceBuilderUrl(id, context) {
  ...
  const portalUrl = getProp(context, 'portalUrl') || DEFAULT_OPTIONS.portalUrl;
  const isPortal = getProp(context, 'isPortal') || false;
  return isPortal
...
function getSurveyUrl(id, context) {
  const survey123Url = getProp(context, 'survey123Url') || DEFAULT_OPTIONS.surveyUrl;
  const portalUrl = getProp(context, 'portalUrl') || DEFAULT_OPTIONS.portalUrl;
...
export { buildItemViewerUrl as b, forceHttps as f, isUrl as i };
