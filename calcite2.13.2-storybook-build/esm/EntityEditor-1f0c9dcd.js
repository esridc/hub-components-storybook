import { H as HubItemEntity, g as getEditorSlug } from './getEditorSlug-78023e22.js';
import { g as getEditorConfig } from './getEditorConfig-a89f031d.js';
import { a as cloneObject } from './util-3e6872d9.js';
import { D as DEFAULT_DISCUSSION, k as editorToContent, l as DEFAULT_PROJECT, m as editorToProject, d as updateProject, n as createProject, o as DEFAULT_INITIATIVE_TEMPLATE, a as updateInitiativeTemplate, p as createInitiativeTemplate, h as deleteInitiativeTemplate, u as updateSurvey, f as deleteSurvey, q as editorToTemplate, b as updateTemplate, r as createTemplate, g as deleteTemplate } from './edit-237c0a70.js';
import { e as enrichEntity } from './enrichEntity-a5bc0b4f.js';
...
import { d as getDownloadConfiguration, g as getCreateReplicaFormats, b as getPagingJobFormats } from './getDownloadConfiguration-6cb6d32f.js';
...
import { a as fetchDiscussion, b as fetchInitiativeTemplate, c as fetchSurvey } from './fetchHubEntity-28d04ab4.js';
import { I as editorToEntity, e as fetchProject, b as fetchInitiative, a1 as DEFAULT_INITIATIVE, C as updateInitiative, a2 as createInitiative, _ as deleteInitiative, a3 as editorToInitiative, f as fetchTemplate } from './HubInitiatives-4f4e24ce.js';
...
import { v as getShortenedCategories } from './compose-d5b83ab7.js';
import { a as getCardModelUrlFromEntity, g as getCardModelUrlFromResult } from './getCardModelUrl-a5543776.js';
import { g as getFamily } from './get-family-543fac52.js';
import { q as upsertResource, t as doesResourceExist } from './themes-e08327b4.js';
...
import { b as convertGroupToHubGroup } from './hubSearch-41612481.js';
...
import { b as buildDefaultEventEntity } from './defaults-1f93a79e.js';
...
import { k as TemplateDefaultFeatures } from './TemplateBusinessRules-0e35d61b.js';
 ...
 */
class HubDiscussion extends HubItemEntity {
    /**
    ...
    static async fetch(identifier, context) {
        try {
            const entity = await fetchDiscussion(identifier, context.hubRequestOptions);
    ...
    static applyDefaults(partialDiscussion, context) {
        ...
        const pojo = Object.assign(Object.assign({}, DEFAULT_DISCUSSION), partialDiscussion);
        return pojo;
    }
    /**
    ...
    async save() {
        ...
        if (this.entity.id) {
            const { updateDiscussion } = await import('./edit-9f487804.js');
            // update it
            this.entity = await updateDiscussion(this.entity, this.context.hubRequestOptions);
        }
        else {
            const { createDiscussion } = await import('./edit-9f487804.js');
            // create it
    ...
    async delete() {
        ...
        const { deleteDiscussion } = await import('./edit-9f487804.js');
    ...
    async fromEditor(editor) {
        ...
        this.entity = editorToEntity(editor, this.context.portal);
        await this.save();
        return this.entity;
    }
}
...
function getDownloadConfigurationDisplayFormats(entity) {
    ...
    if (!flowType && ["Feature Service", "Map Service"].includes(entity.type)) {
        const pagingFormats = getPagingJobFormats().map((f) => {
            return {
                key: f.format,
                hidden: false,
            };
    ...
    if (isHostedFeatureServiceMainEntity(entity) &&
        flowType !== "createReplica") {
        const createReplicaFormats = getCreateReplicaFormats(entity).map((f) => {
            return {
                key: f.format,
                hidden: false,
            };
    ...
    return formats.map((f) => isAdditionalResourceConfiguration(f)
...
class HubContent extends HubItemEntity {
    ...
    async save() {
        ...
        const { createContent, updateContent } = await import('./edit-237c0a70.js').then(function (n) { return n.v; });
        if (this.entity.id) {
            // update it
    ...
    async delete() {
        this._checkDestroyed();
        this.isDestroyed = true;
        const { deleteContent } = await import('./edit-237c0a70.js').then(function (n) { return n.v; });
        // Delegate to module fn
    ...
    async fromEditor(editor) {
        ...
        const entity = editorToContent(editor);
...
function metricToEditor(metric, displayConfig) {
    const { allowExpressionSet, expressionSet, fieldType, itemId, statistic } = displayConfig, config = __rest(displayConfig, ["allowExpressionSet", "expressionSet", "fieldType", "itemId", "statistic"]);
 ...
 * Hub Project Class
 */
class HubProject extends HubItemEntity {
    /**
    ...
    async fromEditor(editor, editorContext) {
        ...
        if (featuredImage) {
            ...
            else if (await doesResourceExist(entity.id, "featuredImage.png", this.context.userRequestOptions)) {
    ...
    async delete() {
        ...
        this.isDestroyed = true;
        const { deleteProject } = await import('./edit-237c0a70.js').then(function (n) { return n.w; });
        // Delegate to module fn
...
const initiativeToCardModel = (initiative, context, opts) => {
    const { actionLinks = [], baseUrl = "", locale = "en-US", target = "self", } = opts || {};
    const titleUrl = getCardModelUrlFromEntity(initiative, context, target, baseUrl);
 ...
 */
class HubInitiative extends HubItemEntity {
    /**
     * Private constructor so we don't have `new` all over the place. Allows for
    ...
    async toEditor(editorContext = {}, include = []) {
        ...
        if (assocGroupId) {
            const associationGroup = await getGroup(assocGroupId, this.context.requestOptions);
            const hubAssociationGroup = convertGroupToHubGroup(associationGroup, this.context.userRequestOptions);
            const _associations = {
...
const initiativeTemplateToCardModel = (initiativeTemplate, context, opts) => {
    const { actionLinks = [], baseUrl = "", locale = "en-US", target = "self", } = opts || {};
    const titleUrl = getCardModelUrlFromEntity(initiativeTemplate, context, target, baseUrl);
 ...
 */
class HubInitiativeTemplate extends HubItemEntity {
    ...
    static async fetch(identifier, context) {
        ...
        try {
            const initiativeTemplate = await fetchInitiativeTemplate(identifier, context.requestOptions);
    ...
    static applyDefaults(partialInitiativeTemplate, context) {
        ...
        const pojo = Object.assign(Object.assign({}, DEFAULT_INITIATIVE_TEMPLATE), partialInitiativeTemplate);
        return pojo;
    }
    /**
     * Convert the project entity into a card view model that can
    ...
    async fromEditor(editor) {
        ...
        delete editor._thumbnail;
        const entity = editorToEntity(editor, this.context.portal);
    ...
    async save() {
        ...
        if (this.entity.id) {
            // update it
            this.entity = await updateInitiativeTemplate(this.entity, this.context.userRequestOptions);
        }
        else {
            // create it
            this.entity = await createInitiativeTemplate(this.entity, this.context.userRequestOptions);
        }
        // call the after save hook on superclass
        await super.afterSave();
    }
    ...
    async delete() {
        ...
        this.isDestroyed = true;
        await deleteInitiativeTemplate(this.entity.id, this.context.userRequestOptions);
    }
}
 ...
 * Hub Survey Class
 */
class HubSurvey extends HubItemEntity {
    /**
 ...
 * @internal
 */
class HubEvent extends HubItemEntity {
    /**
    ...
    static applyDefaults(partialEvent, context) {
        ...
        // extend the partial over the defaults
        const pojo = Object.assign(Object.assign({}, buildDefaultEventEntity()), partialEvent);
        return pojo;
    }
    /**
     * Apply a new state to the instance
    ...
    async save() {
        ...
        if (this.entity.id) {
            const { updateHubEvent } = await import('./edit-fa9666f2.js');
            this.entity = await updateHubEvent(this.entity, this.context.hubRequestOptions);
        }
        else {
            const { createHubEvent } = await import('./edit-fa9666f2.js');
    ...
    async delete() {
        ...
        const { deleteHubEvent } = await import('./edit-fa9666f2.js');
    ...
    async shareWithGroup(groupId) {
        if (!this.context.currentUser) {
            throw new HubError("Share Event With Group", "Cannot share event with group when no user is logged in.");
        }
        this.entity = (await shareEventWithGroups([groupId], this.entity, this.context));
    }
    /**
    ...
    async shareWithGroups(groupIds) {
        this.entity = (await shareEventWithGroups(groupIds, this.entity, this.context));
    }
    /**
    ...
    async unshareWithGroup(groupId) {
        this.entity = (await unshareEventWithGroups([groupId], this.entity, this.context));
    }
    /**
    ...
    async unshareWithGroups(groupIds) {
        this.entity = (await unshareEventWithGroups(groupIds, this.entity, this.context));
    }
    /**
    ...
    async setAccess(access) {
        await updateEvent(Object.assign({ eventId: this.entity.id, data: {
     ...
     */
    async toEditor(editorContext = {}, include = []) {
        const editor = cloneObject(this.entity);
        return editor;
    }
    /**
 ...
 */
class HubTemplate extends HubItemEntity {
    /**
    ...
    static async fetch(identifier, context) {
        try {
            const initiativeTemplate = await fetchTemplate(identifier, context.requestOptions);
...
class EntityEditor {
    ...
    static fromEntity(entity, context) {
        ...
        if (entityType === "site") {
            editor = HubSite.fromJson(entity, context);
        }
        if (entityType === "page") {
            editor = HubPage.fromJson(entity, context);
        }
        ...
        if (entityType === "group") {
            editor = HubGroup.fromJson(entity, context);
        }
...
export { EntityEditor as E, HubContent as H, HubInitiative as a, initiativeResultToCardModel as b, HubInitiativeTemplate as c, initiativeTemplateToCardModel as d, initiativeTemplateResultToCardModel as e, HubDiscussion as f, HubSurvey as g, HubEvent as h, initiativeToCardModel as i, HubProject as j, projectResultToCardModel as k, HubTemplate as l, projectToCardModel as p, removeResource as r };
