...
import { a as addPermissionPolicy, r as removePermissionPolicy } from './enrichEntity-a5bc0b4f.js';
...
import { b as getItemThumbnailUrl } from './compose-d5b83ab7.js';
...
class HubItemEntity {
    ...
    async shareWithGroup(groupId) {
        if (!this.context.currentUser) {
            throw new HubError("Share Item With Group", "Cannot share item with group when no user is logged in.");
        }
    ...
    async getFollowersGroup() {
        ...
        try {
            if (this.entity.followersGroupId) {
                group = await getGroup(this.entity.followersGroupId, this.context.userRequestOptions);
            }
        }
        catch (error) {
            group = null;
    ...
    async setFollowersGroupIsDiscussable(isDiscussable) {
        ...
        const typeKeywords = setDiscussableKeyword(group.typeKeywords, isDiscussable);
        await updateGroup({
            group: {
                id: group.id,
                typeKeywords,
...
export { HubItemEntity as H, deleteItemThumbnail as d, getEditorSlug as g, setItemThumbnail as s, uploadImageResource as u };
