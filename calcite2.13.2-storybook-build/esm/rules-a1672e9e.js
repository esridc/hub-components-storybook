import { j as jsonPointer, g as getPropertyPathFromScope } from './getPropertyFrom-49d414ff.js';
import { S as SCHEMA_TYPE_TO_DEFAULT_FIELD_VALUE } from './resources-3247991b.js';
import { i as isNil } from './is-nil-03b9a6b5.js';
import { a as ajv, c as codegen, A as Ajv } from './ajv-73f98cc1.js';
import { g as getGlobalContext } from './state-31a09db0.js';
import { L as Logger } from './logger-f8667200.js';
import { c as createCommonjsModule, b as getDefaultExportFromCjs } from './_commonjsHelpers-11ca3be1.js';
...
async function validateUniqueDomain(schema, data) {
  ...
  const { lookupDomain } = await import('./index-6cc6aab1.js');
...
async function validateUniqueSlug(schema, data) {
  // NOTE: using dynamic import b/c this keyword
  // is currently added to _all_ validators
  const { findItemsBySlug } = await import('./index-99d5ce48.js');
  const context = getGlobalContext();
...
const addFormats = /*@__PURE__*/getDefaultExportFromCjs(dist);
...
function evaluateCondition(condition, model) {
  ...
  if (condition) {
    // single evaluation
    if (condition.scope) {
      const schema = jsonPointer.get(model.schema, condition.scope);
    ...
    // multiple rules
      ...
      const values = properties.reduce((acc, key) => {
        ...
        const schema = jsonPointer.get(model.schema, scope);
        const value = [
...
export { validate as a, evaluateCondition as b, evaluateConditions as c, evaluateUiSchemaRules as e, instantiateValidator as i, validateAsync as v };
