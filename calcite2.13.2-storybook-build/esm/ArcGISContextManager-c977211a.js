import { a as cloneObject, m as maybeAdd } from './util-3e6872d9.js';
import { a as getEnvironmentFromPortalUrl } from './get-portal-api-url-8aa1582b.js';
...
import { a as Level, L as Logger } from './logger-f8667200.js';
...
async function setUserResource(resource, username, portalUrl, token, replace = false) {
    ...
    return request(`${portalUrl}/sharing/rest/community/users/${username}/addResource`, ro);
}
/**
 * @private
...
function getUserResource(username, key, portalUrl, token) {
    ...
    return request(`${portalUrl}/sharing/rest/community/users/${username}/resources/${key}?token=${token}`, ro);
}
...
function addHistoryEntry(entry, history) {
    ...
    let entries = cloneObject(history.entries);
...
class ArcGISContext {
    ...
    get enterpriseOrgId() {
        if (this._portalSelf) {
            return getProp(this._portalSelf, "portalProperties.hub.settings.enterpriseOrg.orgId");
        }
    }
    /**
    ...
    get isCommunityOrg() {
        let result = false;
        if (this._portalSelf) {
            const orgType = getProp(this._portalSelf, "portalProperties.hub.settings.orgType");
    ...
    async initialize() {
        ...
        if (this._authentication) {
            ...
            if (!this._currentUser) {
                ...
                promises.push(getUser({ username, authentication: this._authentication }));
        ...
        if (this._authentication && hubAppToken) {
            ...
            Object.keys(getWithDefault(this._userHubSettings, "preview", {})).forEach((key) => {
...
export { ArcGISContextManager as A, ArcGISContext as a, ALPHA_ORGS as b, fetchMaxNumUserGroupsLimit as c, getHubApiFromPortalUrl as d, updateUserSiteSettings as e, fetchOrgLimits as f, getOrgThumbnailUrl as g, fetchUserSiteSettings as h, fetchUserHubSettings as i, getObjectSize as j, addHistoryEntry as k, platformSelf as p, removeHistoryEntry as r, updateUserHubSettings as u };
