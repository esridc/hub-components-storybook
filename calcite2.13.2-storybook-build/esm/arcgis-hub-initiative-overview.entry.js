...
const ArcgisHubInitiativesOverview = class {
  ...
  renderLearnAboutNotice() {
    return (h("calcite-notice", { icon: 'lightbulb', kind: 'brand', open: true, width: "full" }, h("arcgis-hub-workspace-link", { href: "https://www.esri.com/arcgis-blog/products/arcgis-hub/announcements/introducing-redefined-initiatives-in-arcgis-hub/", iconEnd: "launch", slot: "link", target: "_blank", telemetry: dist.dictionary.category.navigation.action.external.label.webHelp.details.learnMoreAboutHubInitiatives }, this.intl.t('learnAbout'))));
  }
  renderStatusAndPurpose() {
    return (h("div", { class: 'initiative-overview__status-purpose-container' }, h("calcite-chip", { appearance: "outline", icon: this.entity.status === 'complete' ? 'check' : '', scale: "l", value: this.entity.status }, this.intl.t(`status.${this.entity.status}`)), h("div", { class: 'initiative-overview__purpose' }, this.renderPurpose())));
  }
  renderPurpose() {
    return (this.entity.summary
      ? h("div", null, this.entity.summary)
      : h("div", { class: 'initiative-overview__no-purpose-message' }, this.intl.t('noPurposeSet')));
  }
  renderMap() {
    return (h("arcgis-hub-map", { basemap: 'gray-vector', expand: 1.5, extent: this.extent, graphics: this.graphics, settings: this.mapSettings }, this.hasLocation
      ? null
      : h("div", { class: "initiative-overview__map-overlay" }, h("div", { class: "initiative-overview__no-location-message" }, this.intl.t('noLocationSet')))));
  }
  renderEditButton() {
  ...
  render() {
    return (h(Host, { "data-element": "initiative-overview" }, h("arcgis-hub-workspace-pane", null, h("h1", { slot: "title" }, this.intl.t('overview')), h("div", null, h("div", { class: 'initiative-overview__main' }, this.renderStatusAndPurpose(), this.renderMap(), this.renderEditButton())), h("div", { slot: "side-panel" }, this.renderLearnAboutNotice(), this.shouldShowPageViews && this.renderPageViews(), this.renderMetadata()))));
  }
...
export { ArcgisHubInitiativesOverview as arcgis_hub_initiative_overview };
